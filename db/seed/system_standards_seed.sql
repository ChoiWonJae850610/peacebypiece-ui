...
--
-- 목적:
-- - full_reset 없이 기존 개발 DB에 누락된 시스템 기준정보만 보강한다.
-- - full_reset.sql에는 동일한 시스템 단위/공정/생산품 템플릿 baseline이 이미 포함되어 있다.
-- - 고객사/사용자/작업지시서/협력업체 샘플 데이터는 생성하지 않는다.
--
-- 주의:
-- - full reset 직후에는 이 파일을 추가로 실행할 필요가 없다.
-- - 기존 DB를 유지하면서 시스템 기준정보만 보강해야 할 때만 선택 실행한다.
