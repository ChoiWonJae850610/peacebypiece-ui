...
WITH workorders AS (
  ...
  SELECT
    ...
    w.category2_id,
    w.category3_id,
    format('현실 테스트 작업지시서 %s', w.n),
    format('현실 테스트 작업지시서 %s', w.n),
    w.category1,
    w.category2,
    w.category3,
    CASE WHEN w.n % 2 = 0 THEN 'SS' ELSE 'FW' END,
    CASE WHEN w.n % 5 = 0 THEN '긴급' WHEN w.n % 5 = 1 THEN '높음' ELSE '보통' END,
    w.factory_name,
    CASE WHEN w.n % 3 = 0 THEN '샘플 관리자' WHEN w.n % 3 = 1 THEN '샘플 디자이너' ELSE '샘플 검수담당자' END,
    CASE WHEN w.n % 3 = 0 THEN 'user-sample-admin' WHEN w.n % 3 = 1 THEN 'user-sample-designer' ELSE 'user-sample-inspector' END,
    'user-sample-admin',
    'admin',
    w.due_date,
    w.quantity,
    CASE WHEN w.status = 'completed' THEN w.quantity - (w.n % 6) ELSE 0 END,
    CASE WHEN w.status = 'completed' THEN 'normal' ELSE 'unchecked' END,
    '성능/통계/R2 업로드 검증용 현실형 더미 작업지시서입니다.',
    true,
    w.created_at,
    w.updated_at,
    NULL
