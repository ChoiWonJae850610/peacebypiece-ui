...
-- - 0.9.56~0.13.85에서 추가한 SaaS/초대/온보딩/체험 상태 구조 반영
--
-- 주의:
...
CREATE INDEX signup_applications_correction_due_idx
  ON signup_applications (correction_due_at)
  WHERE status = 'changes_requested';

CREATE UNIQUE INDEX signup_applications_active_email_idx
...
CREATE UNIQUE INDEX signup_applications_active_google_sub_idx
  ON signup_applications (google_sub)
  WHERE status IN ('submitted', 'reviewing', 'changes_requested', 'approved', 'provisioning_failed');

CREATE UNIQUE INDEX signup_applications_active_business_registration_idx
  ON signup_applications (business_registration_number_normalized)
  WHERE status IN ('submitted', 'reviewing', 'changes_requested', 'approved', 'provisioning_failed');

CREATE UNIQUE INDEX signup_applications_created_company_idx
  ON signup_applications (created_company_id)
  WHERE created_company_id IS NOT NULL;

CREATE UNIQUE INDEX signup_applications_created_user_idx
  ON signup_applications (created_user_id)
  WHERE created_user_id IS NOT NULL;

CREATE UNIQUE INDEX signup_applications_created_member_idx
  ON signup_applications (created_company_member_id)
  WHERE created_company_member_id IS NOT NULL;

CREATE UNIQUE INDEX signup_applications_created_subscription_idx
  ON signup_applications (created_subscription_id)
  WHERE created_subscription_id IS NOT NULL;

CREATE INDEX signup_application_files_application_type_active_idx
  ON signup_application_files (application_id, file_type, uploaded_at DESC)
  WHERE deleted_at IS NULL;

CREATE UNIQUE INDEX signup_application_files_storage_key_unique
...
CREATE UNIQUE INDEX policy_versions_one_current_per_document_idx
  ON policy_versions (policy_document_id)
  WHERE is_current = true;

CREATE INDEX policy_versions_current_required_idx
...
CREATE INDEX policy_agreements_company_user_idx
...
VALUES
  ...
  ('workorder.status.order', '작업지시서 발주 상태 변경', '작업지시서 발주 요청 또는 발주 완료 상태를 처리할 수 있다.', 'workflow', 'workflow', 'permissions.workorder.statusOrder.label', 'permissions.workorder.statusOrder.description', false, 70, true),
  ...
  ('material.order.request', '원단·부자재 주문 요청', '담당 작업지시서에서 원단·부자재 주문 요청을 등록하거나 수정할 수 있다.', 'material', 'material', 'permissions.material.orderRequest.label', 'permissions.material.orderRequest.description', false, 100, true),
  ('material.order.place', '원단·부자재 발주 처리', '원단·부자재 발주 처리와 발주 상태 변경을 진행할 수 있다.', 'material', 'material', 'permissions.material.orderPlace.label', 'permissions.material.orderPlace.description', false, 105, true),
...
VALUES
  ...
  ('role-template-company-admin', 'material.order.request', true),
  ('role-template-company-admin', 'material.order.place', true),
  ...
  ('role-template-designer', 'workorder.delete', true),
  ...
  ('role-template-designer', 'stats.read', true),
  ...
  ('role-template-inspector', 'storage.read', true),
  ('role-template-inspector', 'stats.read', true),
  ('role-template-inspector', 'personal_settings.manage', true),
  ('role-template-inventory-manager', 'workorder.read', true),
  ('role-template-inventory-manager', 'material.order.request', true),
  ('role-template-inventory-manager', 'material.order.place', true),
  ...
  ('role-template-inventory-manager', 'partner.delete', true),
  ('role-template-inventory-manager', 'partner.manage', true),
  ...
  ('role-template-inventory-manager', 'standards.delete', true),
  ('role-template-inventory-manager', 'standards.manage', true),
  ('role-template-inventory-manager', 'storage.read', true),
  ('role-template-inventory-manager', 'stats.read', true),
...
CREATE INDEX company_files_company_type_active_idx
  ...
  WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX company_files_storage_key_unique
  ON company_files (storage_key);
CREATE INDEX company_files_review_status_idx
