...
0.9.224342 기준으로 아래 케이스를 명시적으로 포함하고, R2 더미 파일 생성 스크립트와 같은 `attachments.storage_key` 기준으로 매칭된다.
...
이 seed는 attachments metadata만 만든다. 실제 R2 object 파일까지 만들려면 `scripts/seed-r2-demo-files.mjs`를 사용한다. 자세한 PowerShell 실행법은 `scripts/seed-r2-demo-files-usage.md`를 확인한다.
...
- DB schema를 변경하지 않는다.
- `matched_storage_key_count`는 R2 스크립트가 읽을 수 있는 작업지시서 연결 첨부 수다.
