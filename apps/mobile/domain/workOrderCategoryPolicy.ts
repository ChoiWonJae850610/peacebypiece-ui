...
const MAJOR_CODES: Readonly<Record<Exclude<WorkOrderCategoryMajor, "">, string>> = {
  상의: "T",
  하의: "B",
  아우터: "O",
  원피스: "D",
...
const MAJOR_LABELS = Object.fromEntries(
  Object.entries(MAJOR_CODES).map(([label, code]) => [code, label]),
) as Readonly<Record<string, Exclude<WorkOrderCategoryMajor, "">>>;

const LEGACY_MAJOR_BY_PRODUCT_TYPE: Readonly<Record<string, Exclude<WorkOrderCategoryMajor, "">>> = {
  "apparel.top": "상의",
  "apparel.bottom": "하의",
  "apparel.outer": "아우터",
  "apparel.onepiece_set": "원피스",
};
...
export function decodeWorkOrderCategory(input: {
  ...
  const [prefix, targetCode = "", majorCode = "", extra] = code.split("|");
  const structured = prefix === CATEGORY_PREFIX && extra === undefined;
  return {
    targetAudience: structured ? TARGET_LABELS[targetCode] ?? "" : "",
    categoryMajor: structured ? MAJOR_LABELS[majorCode] ?? "" : LEGACY_MAJOR_BY_PRODUCT_TYPE[code] ?? "",
...
export function encodeWorkOrderProductType(input: {
  ...
  const majorCode = MAJOR_CODES[input.categoryMajor as Exclude<WorkOrderCategoryMajor, "">] ?? "";
...
export function workOrderCategorySummary(selection: WorkOrderCategorySelection): string | null {
  const values = [
    ...
    selection.categoryMajor,
    selection.categoryDetail,
    selection.seasonCode,
  ].filter(Boolean);
