...
export function materialErrorMessage(error: unknown): string {
  ...
  if (error.code === "NOT_FOUND" || error.status === 404) return "레시피 또는 원단 정보를 찾을 수 없습니다";
...
export function customerMessage(error: unknown): string {
  ...
  if (error.code === "FORBIDDEN" || error.status === 403) return "레시피를 볼 권한이 없습니다.";
  if (error.code === "NOT_FOUND" || error.status === 404) return "레시피를 찾을 수 없습니다.";
