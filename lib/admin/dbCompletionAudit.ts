...
export const ADMIN_DB_SCREEN_AUDIT_ITEMS: readonly AdminDbScreenAuditItem[] = [
  {
    ...
    routePath: "운영 대시보드",
    status: "empty-state-guarded",
    sourceType: "db-with-empty-state",
    readSource: "작업지시서와 발주 데이터를 기준으로 조회",
    writeSource: "읽기 전용",
    alternateDisplay: "조회 결과가 없거나 실패하면 샘플 수치 없이 빈 운영 통계로 표시",
  ...
  },
  {
    ...
    writeSource: "읽기 전용",
    alternateDisplay: "조회 결과가 없거나 실패하면 샘플 수치 없이 빈 통계로 표시",
  ...
  },
  {
    ...
    alternateDisplay: "조회 결과가 없거나 실패하면 샘플 목록 없이 빈 목록으로 표시",
  ...
  },
  {
    ...
    routePath: "저장소 관리",
    status: "db-prepared",
    sourceType: "db-prepared-empty-state",
    readSource: "첨부파일과 회사 저장소 설정을 기준으로 조회",
    writeSource: "파일 삭제, 복원, 삭제 요청 작업",
    alternateDisplay: "조회 결과가 없거나 실패하면 샘플 파일 없이 기본 저장소 현황으로 표시",
  ...
  },
  {
    ...
    alternateDisplay: "조회 결과가 없거나 실패하면 샘플 협력업체 없이 빈 목록으로 표시",
  ...
  },
  {
    ...
    alternateDisplay: "회사 설정을 불러오지 못하면 회사 기본값 기반의 빈 설정 상태로 표시",
...
const ADMIN_DB_STATUS_PRESENTATION: Record<AdminDbScreenAuditStatus, AdminStatusPresentation> = {
  ...
  "empty-state-guarded": { label: "빈 상태 보호", tone: "info", className: "bg-sky-50 text-sky-700 ring-sky-100" },
  "seed-only": { label: "초기 seed 데이터", tone: "muted", className: "bg-stone-100 text-stone-600 ring-stone-200" },
...
const ADMIN_DB_SOURCE_TYPE_LABELS: Record<AdminDbScreenAuditSourceType, string> = {
  ...
  "db-with-empty-state": "실제 데이터 조회 + 빈 상태 표시",
  "db-prepared-empty-state": "데이터 연결 준비 + 빈 상태 표시",
  "seed-only": "초기 seed 데이터",
...
export function getAdminRepositoryModeLabel(mode: AdminRepositoryMode): string {
  return mode === "db" ? "실제 데이터" : "초기 seed 데이터";
}
