...
type TargetRow = DbQueryResultRow & { work_order_version: number | string; work_order_status: string; total_quantity: number | string; revision_id: string; revision_status: string };
...
async function lockTarget(client: DbTransactionClient, context: MutationContext, input: { scope: TenantMemberScope; workOrderId: WorkOrderId; assignedCompanyMemberId: CompanyMemberId | null }) {
  const selected=await client.query<TargetRow>(`SELECT w.entity_version work_order_version,w.status work_order_status,w.total_quantity,r.id revision_id,r.revision_status FROM work_orders w JOIN work_order_revisions r ON r.company_id=w.company_id AND r.id=w.current_revision_id WHERE w.company_id=$1 AND w.id=$2::uuid AND w.deleted_at IS NULL AND ($3::text IS NULL OR w.assignee_member_id=$3) FOR UPDATE OF w,r`,[input.scope.companyId,input.workOrderId,input.assignedCompanyMemberId]); context.statementCount+=1;
  const row=selected.rows[0]; if(!row)throw new ProcessCommandRepositoryError("not_found");
  if(row.work_order_status!=="draft")throw new ProcessCommandRepositoryError("locked",Number(row.work_order_version));
  if(row.revision_status!=="draft")throw new ProcessCommandRepositoryError("revision_mismatch",Number(row.work_order_version));
  return row;
}
