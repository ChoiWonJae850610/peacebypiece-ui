...
function date(value: unknown): IsoDate | null { return serializePostgresDateOnly(value, "PREVIEW_INVALID_DATE_ONLY"); }
...
export async function getIssuedWorkOrderPreviewV2(input: {
  ...
  return withWaflV2TenantReadOnlyTransaction(async (client) => {
    ...
    const poms = specs.rows.filter((r) => r.kind === "pom").map((r) => ({ id: String(r.a) as never, code: String(r.b), displayName: canonicalPomDisplayName(String(r.b), String(r.c)), displayOrder: count(r.ord) }));
    const matrixTotal = cells.rows.reduce((sum, row) => sum + Number(row.quantity ?? 0), 0);
    const revisionTotal = count(h.total_quantity_snapshot);
