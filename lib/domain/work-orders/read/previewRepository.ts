...
export async function getIssuedWorkOrderPreviewV2(input: {
  ...
  return withWaflV2TenantReadOnlyTransaction(async (client) => {
    ...
    const data: WorkOrderIssuedPreviewReadModel = {
      ...
      header: { workOrderId: String(h.id) as WorkOrderId, revisionId: String(h.revision_id) as WorkOrderRevisionId, productName: String(h.product_name_snapshot), productTypeCode: text(h.product_type_code_snapshot), seasonCode: text(h.season_code_snapshot), itemCode: text(h.item_code_snapshot), dueDate: date(h.due_date_snapshot), totalQuantity: matrixTotal, memo: text(h.memo), factoryDeliveryMemo: text(h.factory_delivery_memo) },
