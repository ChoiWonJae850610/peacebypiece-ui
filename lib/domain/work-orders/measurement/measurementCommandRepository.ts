...
type Target = DbQueryResultRow & { work_order_id: string; revision_id: string; revision_no: number | string; work_order_version: number | string; work_order_status: string; revision_status: string; product_type_code: string | null; item_code: string | null; derivation_kind: string; reorder_round: number | string };
...
export type MeasurementCommandKind = "apply-template" | "set-unit" | "set-cell" | "set-cells" | "add-pom" | "rename-pom" | "remove-pom" | "reorder-poms" | "set-pom-selection" | "save-company-template" | "update-company-template";
...
const commandFor: Record<MeasurementCommandKind, WorkOrderCommandCode> = {
  "apply-template": WORK_ORDER_COMMAND_CODES.measurement.applyTemplate, "set-unit": WORK_ORDER_COMMAND_CODES.measurement.patchUnit, "set-cell": WORK_ORDER_COMMAND_CODES.measurement.patchCell, "set-cells": WORK_ORDER_COMMAND_CODES.measurement.patchCell,
...
async function target(client: DbTransactionClient, context: Context, input: MeasurementCommandInput) {
  ...
  if (row.work_order_status !== "draft" || row.revision_status !== "draft") throw new MeasurementCommandRepositoryError("locked"); if(row.derivation_kind==="reorder"&&Number(row.reorder_round)>0)throw new MeasurementCommandRepositoryError("locked"); return row;
}
...
async function apply(client: DbTransactionClient, context: Context, input: MeasurementCommandInput, row: Target) {
 ...
 if(kind==="set-cells"){
  const raw=Array.isArray(input.payload.cells)?input.payload.cells:[];
  if(raw.length<1||raw.length>250)throw new MeasurementCommandRepositoryError("validation");
  const keys=new Set<string>();
  const cells=raw.map((value)=>{if(!value||typeof value!=="object"||Array.isArray(value))throw new MeasurementCommandRepositoryError("validation");const cell=value as Record<string,unknown>;const workOrderSizeId=uuid(cell.sizeRowId),pomId=uuid(cell.pomColumnId),unit=cell.measurementUnit,text=cell.displayValue;if((unit!=="cm"&&unit!=="inch")||(text!==null&&typeof text!=="string"))throw new MeasurementCommandRepositoryError("validation");const parsed=text===null?null:parseMeasurementToCm(text,unit);if(text!==null&&!parsed)throw new MeasurementCommandRepositoryError("validation");const key=`${workOrderSizeId}:${pomId}`;if(keys.has(key))throw new MeasurementCommandRepositoryError("validation");keys.add(key);return{work_order_size_id:workOrderSizeId,pom_id:pomId,decimal_value:parsed?.centimeters??null,display_fraction:parsed?.displayFraction??null};});
  const changed=await q(client,context,`INSERT INTO work_order_size_spec_values(company_id,revision_id,size_spec_id,size_row_id,pom_column_id,decimal_value,display_fraction,updated_at) SELECT $1,$2::uuid,$3::uuid,spec_size.id,p.id,x.decimal_value,x.display_fraction,now() FROM jsonb_to_recordset($4::jsonb) AS x(work_order_size_id uuid,pom_id uuid,decimal_value numeric,display_fraction text) JOIN work_order_sizes work_size ON work_size.company_id=$1 AND work_size.revision_id=$2::uuid AND work_size.id=x.work_order_size_id JOIN work_order_size_spec_sizes spec_size ON spec_size.company_id=work_size.company_id AND spec_size.revision_id=work_size.revision_id AND spec_size.size_spec_id=$3::uuid AND upper(regexp_replace(trim(spec_size.size_code),'\\s+','','g'))=upper(regexp_replace(trim(work_size.size_code),'\\s+','','g')) JOIN work_order_size_spec_poms p ON p.company_id=spec_size.company_id AND p.size_spec_id=$3::uuid AND p.id=x.pom_id ON CONFLICT(size_spec_id,size_row_id,pom_column_id) DO UPDATE SET decimal_value=EXCLUDED.decimal_value,display_fraction=EXCLUDED.display_fraction,updated_at=now() RETURNING size_row_id`,[input.scope.companyId,row.revision_id,existing.id,JSON.stringify(cells)]);
  if(changed.rows.length!==cells.length)throw new MeasurementCommandRepositoryError("not_found");return ["cells.batch"] as const;
 }
  ...
  if (kind === "save-company-template" || kind === "update-company-template") {
    const predecessorId = kind === "update-company-template" ? uuid(input.payload.templateId) : null;
    const predecessor = predecessorId ? (await q<DbQueryResultRow & { name: string; gender_code: string | null; category_code: string | null; size_set_code: string | null; template_version: number | string }>(client, context, `SELECT name,gender_code,category_code,size_set_code,template_version FROM size_spec_templates WHERE id=$1::uuid AND company_id=$2 AND source_kind='company' AND is_active FOR UPDATE`, [predecessorId, input.scope.companyId])).rows[0] : null;
    if (predecessorId && !predecessor) throw new MeasurementCommandRepositoryError("not_found");
    const templateName = predecessor?.name ?? n(input.payload.templateName);
    const templateVersion = predecessor ? Number(predecessor.template_version) + 1 : 1;
    const templateId = id(`alpha62:company-template:${input.idempotencyKeyHash}`);
    const created = await q<DbQueryResultRow & { id: string }>(client, context, `INSERT INTO size_spec_templates(id,company_id,source_kind,name,gender_code,category_code,size_set_code,template_version,is_active,created_by_member_id) SELECT $1::uuid,$2,'company',$3,COALESCE($4,s.gender_code),COALESCE($5,s.category_code),$6,$7,true,$8 FROM work_order_size_specs s WHERE s.company_id=$2 AND s.id=$9::uuid RETURNING id`, [templateId, input.scope.companyId, templateName, predecessor?.gender_code ?? null, predecessor?.category_code ?? null, predecessor?.size_set_code ?? null, templateVersion, input.scope.companyMemberId, existing.id]);
    if (!created.rows[0]) throw new MeasurementCommandRepositoryError("not_found");
    await q(client, context, `INSERT INTO size_spec_template_sizes(id,template_id,size_code,display_label,display_order) SELECT gen_random_uuid(),$1::uuid,size_code,display_label,display_order FROM work_order_size_spec_sizes WHERE company_id=$2 AND size_spec_id=$3::uuid`, [templateId, input.scope.companyId, existing.id]);
    await q(client, context, `INSERT INTO size_spec_template_poms(id,template_id,pom_code,display_name,measurement_type,instruction,display_order) SELECT gen_random_uuid(),$1::uuid,pom_code,display_name,measurement_type,instruction,display_order FROM work_order_size_spec_poms WHERE company_id=$2 AND size_spec_id=$3::uuid`, [templateId, input.scope.companyId, existing.id]);
    await q(client, context, `INSERT INTO size_spec_template_values(template_id,size_row_id,pom_column_id,decimal_value,display_fraction) SELECT $1::uuid,ts.id,tp.id,v.decimal_value,v.display_fraction FROM work_order_size_spec_values v JOIN work_order_size_spec_sizes s ON s.id=v.size_row_id JOIN work_order_size_spec_poms p ON p.id=v.pom_column_id JOIN size_spec_template_sizes ts ON ts.template_id=$1::uuid AND ts.size_code=s.size_code JOIN size_spec_template_poms tp ON tp.template_id=$1::uuid AND tp.pom_code=p.pom_code WHERE v.company_id=$2 AND v.size_spec_id=$3::uuid`, [templateId, input.scope.companyId, existing.id]);
    if (predecessorId) await q(client, context, `UPDATE size_spec_templates SET is_active=false,updated_at=now() WHERE id=$1::uuid AND company_id=$2 AND source_kind='company' AND is_active`, [predecessorId, input.scope.companyId]);
    await q(client, context, `UPDATE work_order_size_specs SET source_template_id=$3,source_template_version=$4,updated_at=now() WHERE company_id=$1 AND id=$2::uuid`, [input.scope.companyId, existing.id, templateId, templateVersion]);
    return [kind === "update-company-template" ? "companyTemplateVersion" : "companyTemplate", "templateBaseline"] as const;
  }
