...
function map(error: MeasurementCommandRepositoryError): never { const status=error.reason==="not_found"?404:error.reason==="locked"?409:error.reason==="validation"?400:409; throw new WorkOrderCommandRequestError({code:error.reason==="not_found"?"NOT_FOUND":error.reason==="validation"?"VALIDATION_ERROR":error.reason==="locked"?"LOCKED":"CONFLICT",status,message:`Measurement command ${error.reason}.`}); }
