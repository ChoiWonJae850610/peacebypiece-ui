...
export function resolveMakerQaProfile(env = process.env) {
  ...
  if ((definition.id === P.ALPHA64_CURRENT || definition.id === P.ALPHA65_CURRENT) && (
...
export function isCurrentMakerQaProfile(env = process.env) {
  return [P.ALPHA64_CURRENT, P.ALPHA65_CURRENT].includes(resolveMakerQaProfile(env)?.id);
}
