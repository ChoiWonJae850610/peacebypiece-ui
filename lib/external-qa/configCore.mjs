...
function alpha56AccessoryLifecycleParityEnabled(env) {
  return normalized(env.WAFL_EXTERNAL_QA_ALPHA56_ACCESSORY_LIFECYCLE_PARITY_MUTATION_ENABLED) === "true"
    && String(env.WAFL_V2_COMMAND_MUTATION_APPROVED ?? "").trim() === "2.0.0-alpha.56-dev-test-accessory-lifecycle-parity-runtime";
}

function alpha57WorkOrderImageEnabled(env) {
  return normalized(env.WAFL_EXTERNAL_QA_ALPHA57_WORK_ORDER_IMAGE_MUTATION_ENABLED) === "true"
    && normalized(env.WAFL_V2_COMMAND_API_ENABLED) === "1"
    && String(env.WAFL_V2_COMMAND_MUTATION_APPROVED ?? "").trim() === "2.0.0-alpha.57-dev-test-work-order-image-runtime"
    && !isProductionEnvironment(env);
}

function alpha59SizeColorStructureEnabled(env) {
  return normalized(env.WAFL_EXTERNAL_QA_ALPHA59_SIZE_COLOR_STRUCTURE_MUTATION_ENABLED) === "true"
    && normalized(env.WAFL_V2_COMMAND_API_ENABLED) === "1"
    && String(env.WAFL_V2_COMMAND_MUTATION_APPROVED ?? "").trim() === "2.0.0-alpha.59-dev-test-size-color-structure-runtime"
    && !isProductionEnvironment(env);
}

function alpha60DraftChildHardDeleteEnabled(env) {
  return normalized(env.WAFL_EXTERNAL_QA_ALPHA60_DRAFT_CHILD_HARD_DELETE_MUTATION_ENABLED) === "true"
    && normalized(env.WAFL_V2_COMMAND_API_ENABLED) === "1"
    && String(env.WAFL_V2_COMMAND_MUTATION_APPROVED ?? "").trim() === "2.0.0-alpha.60-dev-test-draft-child-hard-delete-runtime"
    && !isProductionEnvironment(env);
}

function alpha61MobileWorkOrderCreateEnabled(env) {
  return normalized(env.WAFL_EXTERNAL_QA_ALPHA61_MOBILE_WORK_ORDER_CREATE_MUTATION_ENABLED) === "true"
    && normalized(env.WAFL_V2_COMMAND_API_ENABLED) === "1"
    && String(env.WAFL_V2_COMMAND_MUTATION_APPROVED ?? "").trim() === "2.0.0-alpha.61-dev-test-mobile-work-order-create-runtime"
    && !isProductionEnvironment(env);
}

function alpha62SizeMeasurementEnabled(env) {
  return normalized(env.WAFL_EXTERNAL_QA_ALPHA62_SIZE_MEASUREMENT_MUTATION_ENABLED) === "true"
    && normalized(env.WAFL_V2_COMMAND_API_ENABLED) === "1"
    && String(env.WAFL_V2_COMMAND_MUTATION_APPROVED ?? "").trim() === "2.0.0-alpha.62-dev-test-size-measurement-runtime"
    && !isProductionEnvironment(env);
}

function makerAuthoringAssetMutationEnabled(env) {
  return alpha57WorkOrderImageEnabled(env) || alpha62SizeMeasurementEnabled(env);
}
...
export function isTailscaleServePathAllowed(pathname, method = "GET", env = process.env) {
  ...
  if (pathname === "/api/v2/work-orders") return verb === "GET" || (verb === "POST" && alpha61MobileWorkOrderCreateEnabled(env));
  ...
  if (/^\/api\/v2\/work-orders\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/size-spec\/commands$/i.test(pathname)) {
    return verb === "POST" && alpha62SizeMeasurementEnabled(env);
  }
  ...
  if (/^\/api\/v2\/size-spec-templates\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(pathname)) {
    return verb === "PATCH" && alpha62SizeMeasurementEnabled(env);
  }
  if (/^\/api\/v2\/work-orders\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/size-color\/(sizes|colors)$/i.test(pathname)) {
    return verb === "POST" && (alpha59SizeColorStructureEnabled(env) || alpha60DraftChildHardDeleteEnabled(env) || alpha61MobileWorkOrderCreateEnabled(env) || alpha62SizeMeasurementEnabled(env));
  }
  ...
  if (/^\/api\/v2\/work-orders\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/size-color\/(sizes|colors)\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(pathname)) {
    if (verb === "DELETE") return alpha60DraftChildHardDeleteEnabled(env) || alpha62SizeMeasurementEnabled(env);
    return verb === "PATCH" && (alpha59SizeColorStructureEnabled(env) || alpha60DraftChildHardDeleteEnabled(env) || alpha61MobileWorkOrderCreateEnabled(env) || alpha62SizeMeasurementEnabled(env));
  }
  ...
  if (/^\/api\/v2\/work-orders\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/size-color\/options$/i.test(pathname)) {
    return verb === "GET" || (verb === "POST" && alpha62SizeMeasurementEnabled(env));
  }
  if (/^\/api\/v2\/work-orders\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/size-color\/options\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(pathname)) {
    return verb === "DELETE" && alpha62SizeMeasurementEnabled(env);
  }
  ...
  if (/^\/api\/v2\/work-orders\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/materials$/i.test(pathname)) {
    ...
    return verb === "POST"
      ...
      && (
        ...
        || (
          normalized(env.WAFL_EXTERNAL_QA_ALPHA55_MATERIAL_ORDER_LIFECYCLE_MUTATION_ENABLED) === "true"
          && String(env.WAFL_V2_COMMAND_MUTATION_APPROVED ?? "").trim() === "2.0.0-alpha.55-dev-test-mobile-material-order-lifecycle-runtime"
        )
        || alpha56AccessoryLifecycleParityEnabled(env)
        || alpha57WorkOrderImageEnabled(env)
        || alpha59SizeColorStructureEnabled(env)
        || alpha60DraftChildHardDeleteEnabled(env)
  ...
  if (/^\/api\/v2\/work-orders\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/materials\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(pathname)) {
    ...
    return verb === "PATCH"
      ...
      && (
        ...
        || (
          normalized(env.WAFL_EXTERNAL_QA_ALPHA52_CORE_INLINE_MUTATION_ENABLED) === "true"
          && String(env.WAFL_V2_COMMAND_MUTATION_APPROVED ?? "").trim() === "2.0.0-alpha.52-dev-test-mobile-core-inline-runtime"
        )
        || (
          normalized(env.WAFL_EXTERNAL_QA_ALPHA55_MATERIAL_ORDER_LIFECYCLE_MUTATION_ENABLED) === "true"
          && String(env.WAFL_V2_COMMAND_MUTATION_APPROVED ?? "").trim() === "2.0.0-alpha.55-dev-test-mobile-material-order-lifecycle-runtime"
  ...
  if (/^\/api\/v2\/work-orders\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/materials\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/(archive|restore)$/i.test(pathname)) {
    return verb === "POST"
      ...
      && (
        (
          normalized(env.WAFL_EXTERNAL_QA_ALPHA51_MATERIAL_LIFECYCLE_MUTATION_ENABLED) === "true"
          && String(env.WAFL_V2_COMMAND_MUTATION_APPROVED ?? "").trim() === "2.0.0-alpha.51-dev-test-mobile-material-lifecycle-runtime"
        )
        || alpha56AccessoryLifecycleParityEnabled(env)
        || alpha57WorkOrderImageEnabled(env)
        || alpha60DraftChildHardDeleteEnabled(env)
