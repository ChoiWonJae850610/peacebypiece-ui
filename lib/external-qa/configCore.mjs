...
export function validatePublicDocumentViewerOrigin(rawOrigin, options = {}) {
  const origin = validateQaOrigin(rawOrigin, { externalQa: true, production: options.production });
  const hostname = new URL(origin).hostname.toLowerCase();
  if (hostname.endsWith(".trycloudflare.com") || hostname.endsWith(".ts.net")) {
    throw new ExternalQaConfigError("PUBLIC_DOCUMENT_VIEWER_BRANDED_ORIGIN_REQUIRED");
  }
  return origin;
}
...
export function readExternalQaServerConfig(env = process.env) {
  ...
  const rawPublicDocumentViewerOrigin = String(env.WAFL_PUBLIC_DOCUMENT_VIEWER_ORIGIN ?? "").trim();
  const publicDocumentViewer = rawPublicDocumentViewerOrigin
    ? (() => {
      const publicOrigin = validatePublicDocumentViewerOrigin(rawPublicDocumentViewerOrigin, { production });
      return { origin: publicOrigin, hostname: new URL(publicOrigin).hostname.toLowerCase() };
    })()
    : null;
  ...
  return { enabled: true, origin, hostname, hostAllowlist, production, runToken, developerAutoConnectEnabled, tailscaleServe, publicDocumentViewer };
}
...
export function isPublicDocumentViewerPathAllowed(pathname, method = "GET") {
...
export function isTailscaleServePathAllowed(pathname, method = "GET", env = process.env) {
  const verb = String(method).toUpperCase();
  // The public Viewer is a hydrated Next application. Its HTML shell is not
  // usable unless the exact framework assets referenced by that shell are
  // available through the same tailnet-only host.
  if (/^\/_next(?:\/|$)/.test(pathname)) return verb === "GET" || verb === "HEAD";
  if (pathname === "/v") return verb === "GET" || verb === "HEAD";
  ...
  if (pathname === "/api/dev/wafl-input-sheet-geometry-evidence") {
    return verb === "POST"
      && normalized(env.WAFL_EXTERNAL_QA_ENABLED) === "true"
      && !isProductionEnvironment(env);
  }
  if (pathname === "/api/dev/a79-stage3a-revoked-object-audit") return verb === "POST" && !isProductionEnvironment(env);
  if (pathname === "/api/dev/a79-stage3b-deleted-object-audit") return verb === "POST" && !isProductionEnvironment(env);
  ...
  if (/^\/api\/v2\/work-orders\/[0-9a-f-]{36}\/copy$/i.test(pathname)) return verb === "POST" && isMakerQaCapabilityEnabled(env, MAKER_QA_CAPABILITY.REORDER_CREATE);
  if (/^\/api\/v2\/work-orders\/[0-9a-f-]{36}\/reorder$/i.test(pathname)) return verb === "GET" || (verb === "POST" && isMakerQaCapabilityEnabled(env, MAKER_QA_CAPABILITY.REORDER_CREATE));
  if (/^\/api\/v2\/work-orders\/[0-9a-f-]{36}\/drawings\/primary-sketch$/i.test(pathname)) {
    return verb === "GET" || (verb === "PATCH" && isMakerQaCapabilityEnabled(env, MAKER_QA_CAPABILITY.DRAWING_AUTHORING));
  }
  if (/^\/api\/v2\/work-orders\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/documents\/preview$/i.test(pathname)) return verb === "GET" && alpha64MakerDocumentR0Enabled(env);
  ...
  if (/^\/api\/v2\/work-orders\/[0-9a-f-]{36}\/documents\/[0-9a-f-]{36}\/revoke$/i.test(pathname)) return verb === "POST" && alpha64MakerDocumentR0Enabled(env);
  if (/^\/api\/v2\/work-orders\/[0-9a-f-]{36}\/documents\/[0-9a-f-]{36}\/purge$/i.test(pathname)) return verb === "POST" && alpha64MakerDocumentR0Enabled(env);
  ...
  if (/^\/api\/v2\/work-orders\/[0-9a-f-]{36}\/images\/[0-9a-f-]{36}\/output-include$/i.test(pathname)) return verb === "PATCH" && alpha64MakerDocumentR0Enabled(env) && makerAuthoringAssetMutationEnabled(env);
  ...
  if (/^\/api\/v2\/work-orders\/documents\/[0-9a-f-]{36}\/access-tokens\/current$/i.test(pathname)) return verb === "GET" && alpha64MakerDocumentR0Enabled(env);
  if (/^\/api\/v2\/work-orders\/documents\/[0-9a-f-]{36}\/viewer-target$/i.test(pathname)) return verb === "GET" && alpha64MakerDocumentR0Enabled(env);
  if (/^\/api\/v2\/work-orders\/documents\/[0-9a-f-]{36}\/health$/i.test(pathname)) return verb === "GET" && alpha64MakerDocumentR0Enabled(env);
  ...
  if (/^\/api\/v2\/work-orders\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/sample$/i.test(pathname)) return verb === "PATCH" && isMakerQaCapabilityEnabled(env, MAKER_QA_CAPABILITY.BASIC_INFO);
  ...
  if (/^\/api\/v2\/work-orders\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/size-color\/quantities\/batch$/i.test(pathname)) {
    return verb === "PATCH" && isMakerQaCapabilityEnabled(env, MAKER_QA_CAPABILITY.SIZE_COLOR_STRUCTURE);
  }
  ...
  if (/^\/api\/v2\/work-orders\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(pathname)) {
    if (verb === "GET") return true;
    if (verb === "DELETE") return isMakerQaCapabilityEnabled(env, MAKER_QA_CAPABILITY.WORK_ORDER_DRAFT_DELETE);
...
export function isExternalQaPathAllowed(pathname, method = "GET", env = process.env) {
  ...
  if (/^\/api\/v2\/work-orders\/[^/]+\/documents\/[^/]+\/revoke$/.test(pathname)) return verb === "POST" && alpha64MakerDocumentR0Enabled(env);
  if (/^\/api\/v2\/work-orders\/[^/]+\/documents\/[^/]+\/purge$/.test(pathname)) return verb === "POST" && alpha64MakerDocumentR0Enabled(env);
  ...
  if (/^\/api\/v2\/work-orders\/[^/]+\/images\/[^/]+\/output-include$/.test(pathname)) return verb === "PATCH" && alpha64MakerDocumentR0Enabled(env) && makerAuthoringAssetMutationEnabled(env);
  if (/^\/api\/v2\/work-orders\/documents\/[^/]+\/access-tokens$/.test(pathname)) return (verb === "GET" || verb === "POST") && alpha64MakerDocumentR0Enabled(env);
  if (/^\/api\/v2\/work-orders\/documents\/[^/]+\/access-tokens\/current$/.test(pathname)) return verb === "GET" && alpha64MakerDocumentR0Enabled(env);
  if (/^\/api\/v2\/work-orders\/documents\/[^/]+\/viewer-target$/.test(pathname)) return verb === "GET" && alpha64MakerDocumentR0Enabled(env);
  if (/^\/api\/v2\/work-orders\/documents\/[^/]+\/health$/.test(pathname)) return verb === "GET" && alpha64MakerDocumentR0Enabled(env);
