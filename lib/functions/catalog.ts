...
export const WAFL_FUNCTION_CATALOG: WaflFunctionItem[] = [
  item({
    ...
    area: "Public Signup",
    route: "/signup",
    title: "Public Trial signup authenticated E2E",
    description: "Dev/test-only applicant and system-admin session fixtures drive the public signup, readiness block, approval, Trial provisioning, and workspace-entry QA flow without automating the Google OAuth screen.",
    category: "normal",
    roles: ["applicant", "system_admin", "company_admin"],
    automationStatus: "automated",
    releaseBlocking: true,
    preconditions: ["approved dev/test DB fingerprint", "synthetic applicant identity", "synthetic system-admin identity"],
    expectedUi: ["signup draft/restore", "submit", "system-admin queue/badge", "readiness block/create", "approval success", "workspace entry"],
    expectedApi: ["fixture route production blocked", "payment readiness API guarded", "approval idempotent"],
    expectedDbChanges: ["synthetic signup fixture rows only"],
    expectedDbUnchanged: ["business data", "production data", "raw card data"],
    permissionRules: ["system-admin queue/review requires actual active system-admin", "applicant can only access own application"],
    tenantContract: {
      actorCompany: "synthetic applicant/company",
      targetCompany: "other synthetic applicant/company",
      assertions: ["applicant IDOR blocked", "company-admin cannot access system queue", "raw certificate URL hidden", "raw payment reference hidden"],
    },
    responsiveContract: {
      viewports: ["chromium-desktop", "webkit-desktop", "mobile-chromium", "mobile-webkit", "ipad-webkit"],
      assertions: ["signup form usable", "system review list/detail usable", "approval actions reachable", "sticky/mobile actions do not overflow"],
    },
    automation: {
      type: "playwright-db",
      filePath: "tests/e2e/public-signup-authenticated.spec.mjs",
      testDataSet: "public-signup-e2e-synthetic",
      lastResult: "not-run",
      profile: "public-signup-authenticated-e2e",
      command: "playwright test tests/e2e/public-signup-authenticated.spec.mjs",
      safety: "dry-run",
      executionNote: "DEV/TEST only. Fixtures are synthetic and must clean DB/R2 residuals to 0.",
    },
  }),
  item({
    id: "PSU-0331-A02",
    order: "0.24.33.1-2",
    area: "System-admin Review",
    route: "/system/signup-applications",
    title: "System-admin queue, readiness, approval, correction, and rejection E2E",
    description: "Covers queue/badge, detail, readiness missing block, fake readiness creation in dev/test, approval, correction/resubmit, reject, duplicate approval, and provisioning failure recovery.",
    category: "permission",
    roles: ["system_admin"],
    automationStatus: "automated",
    releaseBlocking: true,
    preconditions: ["actual active system-admin fixture", "approved dev/test DB fingerprint"],
    expectedUi: ["queue badge", "detail evidence", "readiness status", "safe approval error", "success state"],
    expectedApi: ["system review APIs require actual system-admin", "fake readiness production blocked"],
    expectedDbChanges: ["synthetic review/audit/outbox rows only"],
    expectedDbUnchanged: ["customer business rows", "production data"],
    permissionRules: ["company-admin/member/applicant cannot access review queue", "effective impersonation alone is insufficient"],
    automation: {
      type: "playwright-db",
      filePath: "tests/e2e/public-signup-authenticated.spec.mjs",
      testDataSet: "public-signup-e2e-system-review",
      lastResult: "not-run",
      profile: "public-signup-authenticated-e2e",
      command: "tools/pipeline/peacebypiece-auto-pipeline.ps1 menu 64",
      safety: "dry-run",
      executionNote: "Uses guarded dev/test fixtures and cleanup; no production mutation.",
    },
  }),
  item({
    id: "PSU-0331-A03",
    ...
    route: "/system/signup-applications",
    title: "Payment readiness automation boundary",
    description: "Verifies readiness-missing approval block, dev/test fake readiness creation, production-equivalent fake readiness block, and raw payment reference non-exposure.",
    category: "permission",
    roles: ["system_admin"],
    automationStatus: "automated",
    releaseBlocking: true,
    preconditions: ["submitted signup application", "no real PG integration"],
    expectedUi: ["SIGNUP_APPROVAL_PAYMENT_READINESS_REQUIRED displayed safely", "dev/test readiness button visible only when allowed"],
    expectedApi: ["fake readiness blocked outside dev/test", "raw provider payload not exposed"],
    expectedDbChanges: ["synthetic payment readiness reference only"],
    expectedDbUnchanged: ["raw card number", "CVC", "provider secret", "production payment data"],
    automation: {
      type: "api-db",
      filePath: "scripts/run-public-signup-e2e-integration.mjs",
      testDataSet: "public-signup-readiness-fixture",
      lastResult: "passed",
      profile: "public-signup-authenticated-e2e",
      command: "tools/pipeline/peacebypiece-auto-pipeline.ps1 menu 63",
      safety: "dry-run",
      executionNote: "Approved dev/test DB only; actual PG integration remains false.",
    },
  }),
  item({
    id: "PSU-0331-A04",
    ...
    area: "Responsive",
    route: "/signup",
    title: "Public signup browser matrix and deployed smoke",
    description: "Tracks Chromium, WebKit, mobile, iPad, and deployed read-only smoke coverage while leaving actual Google OAuth and real iOS picker checks as manual QA.",
    category: "responsive",
    roles: ["applicant", "system_admin"],
    automationStatus: "partial",
    releaseBlocking: true,
    preconditions: ["local or deployed read-only target", "no production mutation"],
    expectedUi: ["desktop signup", "mobile signup", "iPad system review", "deployed root/signup no 500"],
    expectedApi: ["dev/test fixture route production blocked", "fake readiness production blocked"],
    expectedDbChanges: [],
    expectedDbUnchanged: ["production DB", "production R2", "Worker"],
    responsiveContract: {
      viewports: ["chromium-desktop", "webkit-desktop", "mobile-chromium", "mobile-webkit", "ipad-webkit"],
      assertions: ["no horizontal overflow", "file input reachable", "approval action reachable", "manual OAuth remains explicit"],
    },
    automation: {
      type: "playwright",
      filePath: "tests/e2e/public-signup-authenticated.spec.mjs",
      testDataSet: "browser-matrix",
      lastResult: "not-run",
      profile: "public-signup-authenticated-e2e",
      command: "playwright test tests/e2e/public-signup-authenticated.spec.mjs",
      safety: "safe",
      executionNote: "Browser artifacts are excluded from source ZIP; deployed smoke is read-only. MANUAL_REQUIRED: real Google OAuth and real iOS file picker.",
    },
  }),
  item({
    id: "PSU-0331-A05",
    ...
    area: "Authorization",
    route: "/signup",
    title: "Public signup certificate and tenant IDOR E2E",
    description: "Tracks PNG/JPEG/PDF certificate upload/viewer, invalid type/oversize/incomplete upload, applicant ownership, system-admin viewer, revoke, linkage, and exact cleanup.",
    category: "tenant",
    roles: ["applicant", "system_admin"],
    automationStatus: "automated",
    releaseBlocking: true,
    preconditions: ["R2 Worker 0.13.71 already deployed", "synthetic certificate fixtures only"],
    expectedUi: ["certificate upload status", "system-admin inline viewer", "safe access errors"],
    expectedApi: ["no raw R2 URL", "no raw storage key", "download path blocked"],
    expectedDbChanges: ["synthetic certificate rows only"],
    expectedDbUnchanged: ["general customer workorder attachments", "production R2 objects"],
    tenantContract: {
      actorCompany: "applicant A",
      targetCompany: "applicant B",
      assertions: ["other applicant certificate blocked", "system-admin viewer remains approval-only", "residual R2 objects 0"],
    },
    automation: {
      type: "api-db",
      filePath: "scripts/run-signup-certificate-r2-integration.mjs",
      testDataSet: "signup-certificate-png-jpeg-pdf",
      lastResult: "passed",
      profile: "public-signup-authenticated-e2e",
      command: "tools/pipeline/peacebypiece-auto-pipeline.ps1 menu 44",
      safety: "confirmation",
      executionNote: "DEV/TEST DB and R2 synthetic fixtures only; exact cleanup required.",
    },
  }),
  item({
  ...
  item({
    id: "ENV-001-D01", order: "7-1", area: "테스트 환경", route: "/functions", title: "dev/test DB·R2·로그인 안전 연결 감사",
    description: "실제 데이터 변경 없이 runtime, DB 식별정보, R2 설정과 test prefix, E2E 로그인 secret, Playwright 대상 URL을 점검한다.", category: "database", roles: ["developer"], automationStatus: "automated", releaseBlocking: true,
    preconditions: ["로컬 또는 dev/test 환경변수 설정"],
    expectedUi: ["/functions에서 감사 항목과 실행 명령 확인"],
    expectedApi: ["DB·R2 접속 없음", "환경 설정 상태 JSON 보고서 생성"],
    expectedDbChanges: [],
    expectedDbUnchanged: ["전체 DB", "전체 R2 객체", "운영 고객 데이터"],
    exceptionCases: ["runtime 미설정 또는 production", "DB 주소가 test/dev로 식별되지 않음", "R2 test prefix 누락", "E2E session secret 누락"],
  ...
  item({
    id: "PDF-MAT-001", order: "8-2", area: "PDF", route: "/workspace/material-orders", title: "공급처 발주 PDF 데이터·출력 정책 계약",
  ...
  item({
    id: "PERF-001-F01", order: "9-1", area: "성능", route: "/worker", title: "작업지시서 1,000건 목록 기준선",
  ...
  item({
    id: "STO-001-S01", order: "10-1", area: "저장용량/R2", route: "/system/storage-usage", title: "회사별 저장용량 비율 계약",
  ...
  item({
    id: "STO-002-S01", order: "10-2", area: "저장용량/R2", route: "/workspace/storage", title: "R2 객체 합계와 DB 집계 reconciliation",
