...
function assertLocalIssuedPdfRenderInput(value) {
  ...
  if (!parsed?.snapshot?.workOrderId
    ...
    || !(parsed.includedAttachmentImages === undefined || (Array.isArray(parsed.includedAttachmentImages)
      && parsed.includedAttachmentImages.every((item) => typeof item?.filename === "string" && typeof item?.dataUrl === "string")))) {
...
export async function removeLocalIssuedPdfRenderInput(runToken) {
  try {
    await fs.unlink(getLocalIssuedPdfRenderInputPath(runToken));
  } catch (error) {
