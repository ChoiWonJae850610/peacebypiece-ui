...
export const issuedWorkOrderPreviewSample = {
  ...
  header: {
    ...
    identity: { isSample: false, derivationKind: "original", reorderRound: 0 },
  },
  ...
  ].map(([processName, partnerName, applicationArea, applicationColorTarget, memo, dueDate], index) => ({ id: `70000000-0000-0000-0000-00000000000${index + 1}`, processTypeCode: index === 0 ? "production_factory" : `STEP_${index + 1}`, processName, partnerId: null, partnerName, quantity: "144", dueDate, unitCode: "장", currency: "KRW", unitPrice: "0", amount: "0", memo, applicationArea, applicationColorTarget, status: "ready", displayOrder: index, editable: false, locked: true, role: index === 0 ? "factory" : "additional" })),
