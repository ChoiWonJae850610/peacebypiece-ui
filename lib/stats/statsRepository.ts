    ...
    async getSystemStats(query?: SystemStatsQuery) {
      ...
      return createStatsSummary({
        ...
        counts: [
          ...
          createCountMetric("storage.used_bytes", "전체 저장용량", 0),
          createCountMetric("storage.company_top", "고객사별 저장용량", 0),
          createCountMetric("plans.starter", "Starter 고객 수", 0),
          createCountMetric("plans.team", "Team 고객 수", 0),
          createCountMetric("plans.business", "Business 고객 수", 0),
          createCountMetric("invitations.created", "초대 생성", 0),
          createCountMetric("invitations.accepted", "초대 수락", 0),
          createCountMetric("invitations.pending", "대기중 초대", 0),
        ],
        ratios: [
          createRatioMetric("companies.active_ratio", "고객사 활성 비율", 0, 0),
          createRatioMetric("invitations.accepted_ratio", "초대 수락률", 0, 0),
        ],
        series: [
          createSeriesMetricPoint("company.storage_usage.unmeasured", "사용량 미집계", 0),
          ...
          createSeriesMetricPoint("invitation.pending", "대기중", 0),
          createSeriesMetricPoint("invitation.accepted", "수락", 0),
          createSeriesMetricPoint("invitation.expired", "만료", 0),
          createSeriesMetricPoint("invitation.revoked", "취소", 0),
        ],
      });
    },
  };
