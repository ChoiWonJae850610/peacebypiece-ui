...
export default function CompanyInvitationJoinRequestPage({
  token,
}: CompanyInvitationJoinRequestPageProps) {
  const tokenPreview = createCompanyInvitationTokenPreview(token);
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-6">
        ...
        <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
          ...
          <article className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-sm leading-6 text-stone-600">
              이메일/비밀번호 직접 구현은 후순위이며, 고객사 신청도 1차 인증은 Google OAuth 기준으로 연결합니다.
            </p>
            <button
              type="button"
              disabled
        ...
        <section className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm">
          <div className="border-b border-stone-100 pb-4">
            ...
            <p className="mt-2 text-sm leading-6 text-stone-600">
              고객사 초대 링크 접속부터 시스템관리자 승인과 고객사 생성까지의 상태 전환 기준입니다.
            </p>
          </div>
