...
interface SystemStatsSuccess {
  ok: true;
  summary: StatsSummary;
}

interface SystemStatsError {
  ok: false;
  error: string;
  message?: string;
}

type SystemStatsResponse = SystemStatsSuccess | SystemStatsError;
...
export default function SystemConsoleShell() {
  ...
  const [message, setMessage] = useState("시스템 통계를 불러오는 중입니다.");
  ...
  useEffect(() => {
    ...
    async function loadSystemStats() {
      ...
      try {
        const response = await fetch("/api/system/stats", { method: "GET" });
        ...
        if (!response.ok || !data.ok) {
          setSummary(null);
          setMessage(data.ok ? "시스템 통계를 불러오지 못했습니다." : data.message || data.error);
          return;
        }

        setSummary(data.summary);
        setMessage("DB 기준 시스템 통계를 불러왔습니다.");
      } catch (error) {
        if (!mounted) {
          return;
        }
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-600">
                고객사, 초대, 요금제, 저장공간, 시스템 통계를 read-only로 확인하는 시스템관리자 홈입니다.
                저장 action, 결제 자동화, audit log write는 이번 버전에서 연결하지 않습니다.
              </p>
            </div>
        ...
        <section className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 className="text-lg font-semibold text-stone-950">시스템 통계 연결 상태</h2>
        ...
        {summary ? (
          ...
          <div className="rounded-2xl border border-dashed border-stone-200 bg-stone-50 px-4 py-8 text-center text-sm text-stone-500">
            {isLoading ? "시스템 통계를 불러오는 중입니다." : "시스템 통계를 표시할 수 없습니다."}
          </div>
        )}

        <section className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm">
          <h2 className="text-lg font-semibold text-stone-950">시스템관리자 메뉴</h2>
        ...
        {summary ? (
          <section className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm">
            <h2 className="text-lg font-semibold text-stone-950">통계 series</h2>
