...
import type { CountMetric, RatioMetric, SeriesMetricPoint, StatsSummary } from "@/lib/stats/statsTypes";

interface SystemStatsApiSuccess {
  ok: true;
  summary: StatsSummary;
}

interface SystemStatsApiError {
  ok: false;
  error: string;
  message?: string;
}

type SystemStatsApiResponse = SystemStatsApiSuccess | SystemStatsApiError;
...
function CountMetricList({ metrics }: { metrics: CountMetric[] }) {
  if (metrics.length === 0) {
    return (
...
export default function SystemStatsReadOnlyPage() {
  const [summary, setSummary] = useState<StatsSummary | null>(null);
  const [message, setMessage] = useState("시스템 통계 상세를 불러오는 중입니다.");
  ...
  useEffect(() => {
    ...
    async function loadSystemStats() {
      ...
      try {
        ...
        const data = (await response.json()) as SystemStatsApiResponse;
        ...
        if (!response.ok || !data.ok) {
          setSummary(null);
          setMessage(data.ok ? "시스템 통계 상세를 불러오지 못했습니다." : data.message || data.error);
          return;
        }

        setSummary(data.summary);
        setMessage("DB 기준 시스템 통계 상세를 불러왔습니다.");
      } catch (error) {
        if (!mounted) {
          return;
        }
  ...
  const otherCounts = useMemo(
    () =>
      summary?.counts.filter(
        (metric) => !primaryMetrics.some((primary) => primary.key === metric.key),
      ) ?? [],
    [primaryMetrics, summary],
  );
    ...
    <main className="min-h-screen bg-stone-50 px-4 py-6 text-stone-900 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-600">
                시스템관리자 통계 API의 count, ratio, series를 read-only로 확인하는 상세 화면입니다.
                chart library 추가 없이 HTML/Tailwind 기반으로 표시합니다.
              </p>
            </div>
        ...
        {summary ? (
          <>
            ...
            <section className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
              <article className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm">
                ...
                <div className="mt-4">
                  <CountMetricList metrics={otherCounts} />
                </div>
              </article>
          ...
          <div className="rounded-2xl border border-dashed border-stone-200 bg-stone-50 px-4 py-8 text-center text-sm text-stone-500">
            {isLoading
              ? "시스템 통계 상세를 불러오는 중입니다."
              : "시스템 통계 상세를 표시할 수 없습니다."}
          </div>
        )}
      </div>
    </main>
