...
function readLoginError(error?: string | null): string | null {
  ...
  if (error === "ACCOUNT_NOT_FOUND") return "아직 WAFL에 등록된 계정이 아닙니다.";
  if (error === "DB_NOT_CONFIGURED") return "데이터베이스 연결 설정이 필요합니다.";
  if (error === "GOOGLE_OAUTH_CLIENT_ID_REQUIRED") return "Google 로그인 Client ID가 아직 연결되지 않았습니다.";
  ...
  if (error === "WAFL_SESSION_SECRET_REQUIRED") return "로그인 세션 보안 설정이 필요합니다.";
...
export default function WaflLoginPage({ error }: WaflLoginPageProps) {
  ...
  return (
    <ATypePublicFrame
      eyebrow="Fashion production workspace"
      title={
        <>
          업무를 연결하고,
          <br />
      ...
      footer={<p>초대받았거나 승인된 사용자만 로그인할 수 있습니다.</p>}
    >
      <ATypePublicCard
        eyebrow="WAFL 로그인"
