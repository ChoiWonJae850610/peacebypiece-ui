...
export default function CompanyCatalogSettingsClient({ initialCatalog }: Props) {
  ...
  return (
    <div className="min-w-0 space-y-6">
      {error ? (
        <div className="break-words rounded border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>
      ) : null}
      <section className="grid gap-3 sm:grid-cols-3">
        <div className="min-w-0 rounded border border-slate-200 bg-white p-4">
          <div className="text-xs text-slate-500">카탈로그 버전</div>
          <div className="mt-1 break-words text-sm font-semibold text-slate-900">{catalog.catalogVersionCode ?? "미생성"}</div>
        </div>
        <div className="rounded border border-slate-200 bg-white p-4">
          <div className="text-xs text-slate-500">활성 분류</div>
          <div className="mt-1 text-2xl font-semibold text-slate-900">{categories.filter((item) => item.isEnabled).length}</div>
        </div>
        <div className="rounded border border-slate-200 bg-white p-4">
          <div className="text-xs text-slate-500">POM 템플릿</div>
          <div className="mt-1 text-2xl font-semibold text-slate-900">{catalog.poms.length}</div>
        </div>
      </section>

      <section className="overflow-hidden rounded border border-slate-200 bg-white">
        <div className="border-b border-slate-200 px-4 py-3 text-sm font-semibold text-slate-900">회사 분류</div>
        <div className="divide-y divide-slate-100">
          {categories.map((category) => (
            <div key={category.code} className="grid min-w-0 gap-2 px-4 py-3 text-sm sm:grid-cols-[minmax(0,1fr)_120px_120px_110px]">
