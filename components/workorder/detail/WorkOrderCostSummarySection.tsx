...
const COST_ROW_CLASS = "pbp-cost-row flex min-w-0 items-center justify-between gap-4 rounded-[var(--pbp-radius-wafl)] px-3 py-2";
    ...
    <DesktopCostSummaryLayout
      left={<SummaryCard title={copy.summaryTitle || undefined}>
        <div className="space-y-2 text-sm">
          <div data-wafl-component="info-card" className="pbp-cost-grand-total rounded-[var(--pbp-radius-wafl)] border px-4 py-3">
          ...
          <div data-wafl-component="info-row" className={COST_ROW_CLASS}><span className={COST_LABEL_CLASS}>{copy.fabricTotal}</span><span className={getCostValueClassName(fabricTotal)}>{formatCurrency(fabricTotal, common.currencySuffix)}</span></div>
          <div data-wafl-component="info-row" className={COST_ROW_CLASS}><span className={COST_LABEL_CLASS}>{copy.subsidiaryTotal}</span><span className={getCostValueClassName(subsidiaryTotal)}>{formatCurrency(subsidiaryTotal, common.currencySuffix)}</span></div>
          <div data-wafl-component="info-row" className={COST_ROW_CLASS}><span className={COST_LABEL_CLASS}>{copy.laborCost}</span><span className={COST_VALUE_CLASS}>{formatCurrency(costSummary.processLaborCost, common.currencySuffix)}</span></div>
          <div data-wafl-component="info-row" className={COST_ROW_CLASS}><span className={COST_LABEL_CLASS}>{copy.lossCost}</span><span className={COST_VALUE_CLASS}>{formatCurrency(costSummary.processLossCost, common.currencySuffix)}</span></div>
        </div>
      </SummaryCard>}
      right={<SummaryCard title={copy.processTitle}>
        <div className="space-y-2 text-sm">
          {costSummary.processLines.length > 0 ? costSummary.processLines.map((item) => (
            <div key={item.id} data-wafl-component="info-row" className="pbp-cost-row rounded-[var(--pbp-radius-wafl)] px-3 py-2">
          ...
          )) : <div data-wafl-component="empty-card" className="pbp-empty-state rounded-[var(--pbp-radius-wafl)] border border-dashed px-4 py-6 text-center text-sm">{copy.empty}</div>}
        </div>
      </SummaryCard>}
    />
  );
