...
function getCostSummaryValues(params: {
...
function getCanOpenInspectionModal(params: {
...
function getProductionSectionOpen(materialOpen: boolean, outsourcingOpen: boolean): boolean {
  return materialOpen || outsourcingOpen;
}

function getVendorOptions(partnerOptions: string[], materialItems: Material[], outsourcingItems: Outsourcing[]): string[] {
...
export default function WorkOrderDetail({
  ...
  const { laborCost, lossCost, totalCost: totalCostWithOrderInfo, unitCost: unitCostWithOrderInfo } = getCostSummaryValues({
    orderItems,
    fabricTotal,
    subsidiaryTotal,
    outsourcingTotal,
  });
  const canOpenInspectionModal = getCanOpenInspectionModal({
    canEditInventory,
    currentWorkflowState,
    orderItems,
  });
  const productionSectionOpen = getProductionSectionOpen(materialOpen, outsourcingOpen);
  const vendorOptions = getVendorOptions(partnerOptions, materialItems, outsourcingItems);
