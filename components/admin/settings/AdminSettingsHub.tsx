...
function resolveRequestStatusLabel(
  ...
  if (status === "reviewing")
    return t("settings.accountRequest.status.reviewing", "검토 중");
  if (status === "approved")
    return t("settings.accountRequest.status.approved", "승인됨");
  if (status === "rejected")
    return t("settings.accountRequest.status.rejected", "반려됨");
  if (status === "cancelled")
    return t("settings.accountRequest.status.cancelled", "취소됨");
...
function resolveRequestTypeLabel(
  ...
  if (type === "account_deactivation")
    return t(
      "settings.accountRequest.type.accountDeactivation",
      "계정 비활성화",
    );
...
function BillingPlanPanel({
  ...
  const subscriptionStatusTone = subscription
    ? resolveSubscriptionStatusTone(subscription.status)
    : "neutral";
  const storageLimitLabel = subscription
    ? formatStorageBytes(subscription.storageLimitBytes)
    : "-";
  const storageUsedLabel = subscription
    ? formatStorageBytes(subscription.storageUsedBytes)
    : "-";
  const storageUsageLabel = subscription
    ? formatPercent(subscription.storageUsageRatio)
    : "-";
  const storageUsageTone: AdminStatusBadgeTone =
    subscription && subscription.storageUsageRatio >= 1
      ? "danger"
  ...
  const memberUsageLabel = subscription
    ...
    : "-";
  const memberUsageTone: AdminStatusBadgeTone =
    subscription && subscription.activeMemberCount > subscription.memberLimit
      ? "warning"
      : "success";
  const planEndsAt =
    subscription?.currentPeriodEndsAt ?? subscription?.trialEndsAt ?? null;
  const sourceLabel =
    subscription?.source === "company_subscriptions"
  ...
  const isSubscriptionFailed =
    subscriptionLoadState === "failed" || loadState === "failed";
  ...
  return (
    <WaflSectionPanel
      ...
      description={t(
        "settings.billing.redesignedDescription",
        "현재 요금제, 저장공간, 멤버 한도만 빠르게 확인하고 필요한 변경은 바로 요청합니다.",
      )}
      actions={
        <>
          <AdminStatusBadge tone={subscriptionStatusTone}>
            {subscription?.planLabel ?? overview.currentPlanLabel}
          </AdminStatusBadge>
          <AdminStatusBadge tone={subscriptionStatusTone}>
            {subscription?.statusLabel ?? overview.billingStatusLabel}
          </AdminStatusBadge>
        </>
      }
    ...
    >
      {isSubscriptionFailed ? (
        <WaflSettingCard
          title={t(
            "settings.billing.subscriptionFailedTitle",
            "요금제 데이터를 불러오지 못했습니다.",
          )}
          description={t(
            "settings.billing.subscriptionFailedDescription",
            "회사 구독 데이터 조회에 실패했습니다. 기존 환경설정 요약값을 임시로 표시합니다.",
          )}
          tone="warning"
          density="compact"
        />
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(300px,0.75fr)]">
        <WaflSettingsSectionGroup
          ...
          title={t(
            "settings.billing.redesignedSummaryTitle",
            "요금제·저장공간 현황",
          )}
          description={t(
            "settings.billing.redesignedSummaryDescription",
            "고객사가 지금 사용하는 요금제와 한도만 표시합니다.",
          )}
          badge={
            <AdminStatusBadge
              tone={subscriptionLoadState === "failed" ? "warning" : "neutral"}
              size="xs"
            >
        ...
        >
          <div className="grid gap-3 lg:grid-cols-3">
            <WaflSurface shape="control" tone="surface" className="p-4">
              <p className="text-xs font-bold text-[var(--pbp-text-subtle)]">
                {t("settings.billing.currentPlanLabel", "현재 요금제")}
              </p>
              <div className="mt-2 flex flex-wrap items-center gap-2">
                <strong className="text-xl font-black tracking-[-0.04em] text-[var(--pbp-text-primary)]">
                  {subscription?.planLabel ?? overview.currentPlanLabel}
                </strong>
                {subscription ? (
              ...
              <p className="mt-3 text-xs leading-5 text-[var(--pbp-text-muted)]">
                {t("settings.billing.planEndLabel", "요금제 종료일")}:{" "}
                {formatSettingsDateTime(planEndsAt)}
              </p>
            </WaflSurface>
            ...
            <WaflSurface shape="control" tone="surface" className="p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-xs font-bold text-[var(--pbp-text-subtle)]">
                    {t("settings.billing.storageUsageLabel", "저장공간 사용량")}
                  </p>
                  <strong className="mt-2 block text-xl font-black tracking-[-0.04em] text-[var(--pbp-text-primary)]">
                    {storageUsageLabel}
                  </strong>
                </div>
                <AdminStatusBadge tone={storageUsageTone} size="xs">
              ...
              <div className="mt-4 h-2 overflow-hidden rounded-full bg-[var(--pbp-surface-muted)]">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, Math.max(0, Math.round((subscription?.storageUsageRatio ?? 0) * 100)))}%`,
                  }}
                />
              </div>
              <p className="mt-3 text-xs leading-5 text-[var(--pbp-text-muted)]">
                {t(
                  "settings.billing.storageUsageDescription",
                  "활성 회사 파일 기준의 저장공간 사용량입니다.",
                )}
              </p>
            </WaflSurface>
            ...
            <WaflSurface shape="control" tone="surface" className="p-4">
              <p className="text-xs font-bold text-[var(--pbp-text-subtle)]">
                {t("settings.billing.memberUsageLabel", "멤버 사용량")}
              </p>
              <div className="mt-2 flex flex-wrap items-center gap-2">
                <strong className="text-xl font-black tracking-[-0.04em] text-[var(--pbp-text-primary)]">
                  {memberUsageLabel}
                </strong>
                <AdminStatusBadge tone={memberUsageTone} size="xs">
                  {memberUsageTone === "warning"
                    ? t("settings.billing.memberOverLimit", "초과")
                    : t("settings.billing.withinLimit", "한도 내")}
                </AdminStatusBadge>
              </div>
              <p className="mt-3 text-xs leading-5 text-[var(--pbp-text-muted)]">
                {t(
                  "settings.billing.memberUsageDescription",
                  "현재 고객사 활성 멤버 수와 요금제 멤버 한도입니다.",
                )}
              </p>
            </WaflSurface>
          </div>
          ...
          >
            {[
              [
                t("settings.billing.statusLabel", "구독 상태"),
                subscription?.statusLabel ?? overview.billingStatusLabel,
              ],
              [
                t("settings.billing.planEndLabel", "요금제 종료일"),
              ...
              [
                t("settings.billing.updatedAtLabel", "최근 갱신"),
                formatSettingsDateTime(subscription?.updatedAt ?? null),
              ],
              [t("settings.billing.sourceLabel", "데이터 기준"), sourceLabel],
            ].map(([label, value]) => (
              <div
                key={label}
                className="grid gap-1 border-b border-[var(--pbp-border)] px-4 py-3 last:border-b-0 sm:grid-cols-[150px_minmax(0,1fr)] sm:gap-4"
              >
                <span className="text-xs font-bold text-[var(--pbp-text-subtle)]">
                  {label}
                </span>
                <span className="text-sm font-semibold text-[var(--pbp-text-primary)]">
                  {value}
                </span>
              </div>
            ))}
        ...
        <WaflSettingsSectionGroup
          ...
          description={t(
            "settings.billing.actionsDescription",
            "필요한 항목을 선택해 변경 요청 또는 결제 문의로 이어갑니다.",
          )}
          tone="info"
        >
            ...
            {normalizedActions.map((action) => (
              ...
              >
                <span className="min-w-0">
                  <span className="block text-sm font-bold text-[var(--pbp-text-primary)]">
                    {action.label}
                  </span>
                  <span className="mt-1 block line-clamp-1 text-xs text-[var(--pbp-text-muted)]">
                    {action.description}
                  </span>
                </span>
                <AdminStatusBadge tone="neutral" size="xs">
          ...
          <p className="mt-3 text-xs leading-5 text-[var(--pbp-text-muted)]">
            {t(
              "settings.billing.actionsFootnote",
              "정식 결제창 연결 전까지 변경 요청은 운영팀 검토 흐름으로 접수됩니다.",
            )}
          </p>
        </WaflSettingsSectionGroup>
      </div>
...
function AccountSettingsPanel({
  ...
  >(null);
  const [activeRequestMetricId, setActiveRequestMetricId] = useState<
    string | null
  >(null);
  ...
  const [requestFeedbackTone, setRequestFeedbackTone] =
    useState<ToastTone>("info");
  const [requestFeedbackEventKey, setRequestFeedbackEventKey] = useState(0);

  const deactivationAction =
    overview.actions.find(
      (action) => action.requestType === "account_deactivation",
    ) ?? null;
  const activeRequestMetric =
    overview.metrics.find((metric) => metric.id === activeRequestMetricId) ??
    null;
  const editableMetricIds = useMemo(
    () =>
      new Set([
        "company-name",
  ...
  const canSubmitRequest =
    requestState !== "submitting" && requestMessage.trim().length >= 10;
  ...
  const submitAccountRequest = async () => {
    ...
    const message =
      activeRequestType === "company_info_change" && activeRequestMetric
        ? [
    ...
    try {
      ...
      setRequestFeedback(
        t(
          "settings.accountRequest.submitted",
          "요청이 접수되었습니다. 시스템관리자 검토 후 처리됩니다.",
        ),
      );
      setRequestMessage("");
      setActiveRequestType(null);
      ...
      setRequestFeedback(
        t(
          "settings.accountRequest.failed",
  ...
  return (
    <WaflSectionPanel
      ...
      title={overview.title}
      description={t(
        "settings.account.redesignedDescription",
        "회사 정보와 대표 계정 상태를 확인하고, 필요한 변경은 요청으로 접수합니다.",
      )}
      actions={
        <>
          <AdminStatusBadge tone={overview.statusTone}>
            {overview.statusLabel}
          </AdminStatusBadge>
          <AdminStatusBadge
            tone={loadState === "failed" ? "warning" : "neutral"}
          >
            {loadState === "loading"
              ? t("common.loadingShort", "조회 중")
              : t("settings.account.currentData", "현재 계정 기준")}
          </AdminStatusBadge>
        </>
      }
      className="min-h-[320px]"
    ...
    >
      <WaflSettingsSectionGroup
        ...
        description={t(
          "settings.account.companyFieldRequestDescription",
          "변경이 필요한 항목의 수정 버튼을 눌러 요청을 접수합니다.",
        )}
        badge={
          <AdminStatusBadge tone={overview.statusTone}>
            {overview.statusLabel}
        ...
        aside={
          deactivationAction ? (
            <AdminButton
              ...
              size="sm"
              variant="danger"
              onClick={openDeactivationRequest}
            >
        ...
        tone="neutral"
      >
        <WaflSurface shape="control" tone="surface" className="overflow-hidden">
          {overview.metrics.map((metric) => {
            ...
            return (
              ...
              >
                <div className="text-xs font-bold text-[var(--pbp-text-subtle)]">
                  {metric.label}
                </div>
                <div className="min-w-0">
                  <div className="break-words text-sm font-bold text-[var(--pbp-text-primary)]">
                    {metric.value}
                  </div>
                  <div className="mt-1 break-words text-xs leading-5 text-[var(--pbp-text-muted)]">
                    {metric.description}
                  </div>
                </div>
                {editable ? (
      ...
      <ToastMessage
        message={requestFeedback || null}
        tone={requestFeedbackTone}
        eventKey={requestFeedbackEventKey}
      />

      <WaflSettingsSectionGroup
        eyebrow={t("settings.accountRequest.historyEyebrow", "요청 이력")}
        title={t(
          "settings.accountRequest.historyTitle",
          "요청 이력과 처리 결과",
        )}
        description={t(
          "settings.accountRequest.historyDescription",
          "회사 정보 변경과 탈퇴 요청의 접수 상태와 처리 결과를 확인합니다.",
        )}
        badge={
          <AdminStatusBadge
            tone={requestsLoadState === "failed" ? "warning" : "neutral"}
            size="xs"
          >
            {requestsLoadState === "loading"
              ? t("common.loadingShort", "조회 중")
              : t("settings.accountRequest.historyBadge", "최근 5건")}
          </AdminStatusBadge>
        }
        tone="neutral"
      >
        {requestsLoadState === "failed" ? (
          <WaflSettingCard
            title={t(
              "settings.accountRequest.historyFailedTitle",
              "요청 이력을 불러오지 못했습니다.",
            )}
            description={t(
              "settings.accountRequest.historyFailedDescription",
              "요청 접수 기능은 계속 사용할 수 있습니다. 이력은 잠시 후 다시 확인해 주세요.",
            )}
            tone="warning"
            density="compact"
          />
          ...
          <div className="grid gap-3">
            {requests.map((request) => (
              <WaflSettingCard
                ...
                title={
                  request.requestType === "account_deactivation"
                    ? t("settings.accountRequest.withdrawalButton", "탈퇴 요청")
                ...
                badge={
                  <AdminStatusBadge
                    tone={requestStatusTone[request.requestStatus]}
                    size="xs"
                  >
                    {resolveRequestStatusLabel(request.requestStatus, t)}
                  </AdminStatusBadge>
                }
                ...
                footer={
                  <div className="grid gap-1 text-xs leading-5 text-[var(--pbp-text-muted)]">
                    <span>
                      {t("settings.accountRequest.reviewerLabel", "검토자")}:{" "}
                      {request.reviewerName?.trim() || "-"}
                    </span>
                    <span>
                      {t("settings.accountRequest.reviewedAtLabel", "검토일")}:{" "}
                      {request.reviewedAt
                        ? formatSettingsRequestDate(request.reviewedAt)
                        : "-"}
                    </span>
                    {request.reviewMessage?.trim() ? (
                      <span>
                        {t(
                          "settings.accountRequest.reviewMessageLabel",
                          "검토 메모",
                        )}
          ...
          <WaflSettingCard
            title={t(
              "settings.accountRequest.historyEmptyTitle",
              "접수된 요청이 없습니다.",
            )}
            description={t(
              "settings.accountRequest.historyEmptyDescription",
              "회사 정보 변경 또는 탈퇴가 필요할 때 요청을 작성하면 이곳에 최근 이력이 표시됩니다.",
            )}
            tone="neutral"
            density="compact"
          />
      ...
      <AdminModal
        ...
        description={
          activeRequestType === "account_deactivation"
            ? t(
                "settings.accountRequest.withdrawalDescription",
                "탈퇴 요청이 승인되면 서비스 이용이 제한됩니다. 필요한 파일은 미리 내려받아 주세요.",
              )
            : t(
                "settings.accountRequest.fieldModalDescription",
                "현재 값을 기준으로 변경할 내용과 사유를 적어 요청합니다. 시스템관리자 검토 후 처리됩니다.",
              )
        }
          ...
          <div className="flex w-full flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            ...
            >
              {requestState === "submitting"
                ...
                : t("settings.accountRequest.submit", "요청 접수")}
            </AdminButton>
          </div>
        }
      >
        {activeRequestMetric ? (
          <div className="wafl-shape-control border border-[var(--pbp-border)] bg-[var(--pbp-surface-soft)] px-4 py-3">
            <div className="text-xs font-bold text-[var(--pbp-text-subtle)]">
              {activeRequestMetric.label}
            </div>
            <div className="mt-1 break-words text-sm font-black text-[var(--pbp-text-primary)]">
              {activeRequestMetric.value}
            </div>
            <div className="mt-1 text-xs leading-5 text-[var(--pbp-text-muted)]">
              {activeRequestMetric.description}
            </div>
          </div>
        ) : null}
        <label className="grid gap-2">
          <span className="text-xs font-bold text-[var(--pbp-text-subtle)]">
            {activeRequestType === "account_deactivation"
              ? t(
                  "settings.accountRequest.withdrawalReasonLabel",
                  "탈퇴 요청 사유",
                )
              : t(
                  "settings.accountRequest.changeContentLabel",
                  "변경 내용과 사유",
                )}
          </span>
          <textarea
            ...
            rows={6}
            className="min-h-36 w-full wafl-shape-control border border-[var(--pbp-border)] bg-[var(--pbp-surface)] px-3 py-2 text-sm leading-6 text-[var(--pbp-text-primary)] outline-none transition focus:border-[var(--pbp-focus-ring)] focus:ring-2 focus:ring-[var(--pbp-focus-ring)]/20"
            placeholder={
              activeRequestType === "account_deactivation"
                ? t(
                    "settings.accountRequest.withdrawalPlaceholder",
                    "예: 서비스 이용을 중단하려고 합니다. 필요한 자료를 내려받은 뒤 탈퇴 처리를 요청합니다.",
                  )
                : t(
                    "settings.accountRequest.fieldPlaceholder",
                    "예: 새 회사명은 WAFL Sample Co. 입니다. 사업자등록증과 동일하게 변경 요청합니다.",
                  )
            }
          />
        </label>
        <p className="text-xs leading-5 text-[var(--pbp-text-muted)]">
          {t(
            "settings.accountRequest.validation",
...
function MarkdownDocumentBody({ markdown }: { markdown: string }) {
  ...
  const flushList = () => {
    ...
    elements.push(
      <ul
        key={key}
        className="list-disc space-y-2 wafl-shape-control border border-[var(--pbp-border)] bg-[var(--pbp-surface-soft)] px-6 py-4 text-sm leading-7 text-[var(--pbp-text-muted)]"
      >
  ...
  blocks.forEach((rawLine, index) => {
    ...
    if (line.startsWith("---")) {
      flushList();
      elements.push(
        <hr key={`hr-${index}`} className="border-[var(--pbp-border)]" />,
      );
      return;
    }
    ...
    if (line.startsWith("### ")) {
      ...
      elements.push(
        <h4
          key={`h4-${index}`}
          className="pt-2 text-sm font-black text-[var(--pbp-text-primary)]"
        >
          {line.slice(4)}
        </h4>,
      );
    ...
    if (line.startsWith("## ")) {
      ...
      elements.push(
        <h3
          key={`h3-${index}`}
          className="pt-4 text-base font-black text-[var(--pbp-text-primary)]"
        >
          {line.slice(3)}
        </h3>,
      );
    ...
    if (line.startsWith("# ")) {
      ...
      elements.push(
        <h2
          key={`h2-${index}`}
          className="text-lg font-black text-[var(--pbp-text-primary)]"
        >
          {line.slice(2)}
        </h2>,
      );
    ...
    elements.push(
      <p
        key={`p-${index}`}
        className="text-sm leading-7 text-[var(--pbp-text-muted)]"
      >
        {line}
      </p>,
    );
...
function SettingsNoticePanel({ noticeId }: { noticeId: "legal" }) {
  ...
  const [selectedDocument, setSelectedDocument] =
    useState<CustomerPolicyDocument | null>(null);
  const [selectedDocumentContent, setSelectedDocumentContent] =
    useState<CustomerPolicyMarkdownDocument | null>(null);
  const [selectedDocumentLoadState, setSelectedDocumentLoadState] = useState<
  ...
  const openDocumentModal = useCallback(
    async (document: CustomerPolicyDocument) => {
      ...
      try {
        const response = await fetch(
          `/api/policies/customer-documents/${encodeURIComponent(document.id)}`,
          { cache: "no-store" },
        );
        const payload = (await response
          .json()
          .catch(() => ({}))) as CustomerPolicyMarkdownPayload;
        ...
        console.error(
          "[settings] failed to load customer policy markdown",
          error,
        );
  ...
  return (
    <>
      <WaflSectionPanel
        ...
        description={t(
          "settings.notice.redesignedDescription",
          "필수 약관과 운영정책을 이 화면에서 바로 확인합니다. 각 문서를 누르면 전체 내용을 모달로 열람합니다.",
        )}
        actions={
          <>
            <AdminStatusBadge tone="warning">
              {t("settings.notice.requiredCount", "필수 동의")}{" "}
              {requiredPolicyCount}
              {t("common.countUnit", "건")}
            </AdminStatusBadge>
            <AdminStatusBadge tone="neutral">
              {CUSTOMER_POLICY_DOCUMENTS.length}
              {t("common.countUnit", "건")}
            </AdminStatusBadge>
          </>
        }
      ...
      >
        <WaflSettingsSectionGroup
          ...
          description={t(
            "settings.notice.documentListDescription",
            "문서별 필수 여부와 버전을 확인하고 필요한 문서를 바로 열람합니다.",
          )}
          tone="neutral"
        >
          <WaflSurface
              ...
              >
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="text-sm font-black text-[var(--pbp-text-primary)]">
                      {document.title}
                    </h3>
                    <AdminStatusBadge
                      tone={policyCategoryTone[document.category]}
                      size="xs"
                    >
                    ...
                    {document.requiredForApproval ? (
                      <AdminStatusBadge tone="warning" size="xs">
                        {t("settings.notice.required", "필수 동의")}
                      </AdminStatusBadge>
                    ) : null}
                  </div>
                  ...
                  <p className="mt-2 line-clamp-2 text-xs leading-5 text-[var(--pbp-text-muted)]">
                    {document.summary}
                  </p>
                </div>
                <div className="flex items-center justify-between gap-3 lg:justify-end">
                  ...
                  <AdminButton
                    ...
                    size="sm"
                    variant="secondary"
                    onClick={() => void openDocumentModal(document)}
                  >
        ...
        <WaflSettingsSectionGroup
          ...
          description={t(
            "settings.notice.agreementDescription",
            "중요 정책 변경이나 재동의 필요 상태는 업무 접근 차단 화면과 약관·정책 화면에서 함께 안내됩니다.",
          )}
          badge={
            <AdminStatusBadge tone="warning" size="xs">
              {t("settings.notice.required", "필수 동의")}
        ...
        >
          <div className="grid gap-2 text-sm leading-6 text-[var(--pbp-text-muted)] sm:grid-cols-2">
            <p>
              {t(
                "settings.notice.requiredPolicySummary",
                "필수 문서는 이용약관, 개인정보처리방침, 요금·환불정책, 데이터 보관·삭제정책입니다.",
              )}
            </p>
            <p>
              {t(
                "settings.notice.reagreementSummary",
                "재동의가 필요한 경우 이 탭에서 문서를 열람한 뒤 약관·정책 화면에서 동의 상태를 갱신합니다.",
              )}
            </p>
          </div>
        </WaflSettingsSectionGroup>
      ...
      <AdminModal
        ...
        title={
          ...
          t("settings.notice.documentModalTitle", "약관 문서")
        }
        description={
          selectedDocument
            ? `${selectedDocument.subtitle} · ${selectedDocument.versionLabel}`
            : undefined
        }
      ...
      >
        {selectedDocument ? (
          <div className="space-y-4">
            <div className="wafl-shape-control border border-[var(--pbp-border)] bg-[var(--pbp-surface-soft)] p-4">
              <div className="flex flex-wrap items-center gap-2">
                <AdminStatusBadge
                  tone={policyCategoryTone[selectedDocument.category]}
                >
                  {selectedDocument.categoryLabel}
                </AdminStatusBadge>
                <AdminStatusBadge tone="neutral">
                  {selectedDocument.versionLabel}
                </AdminStatusBadge>
                <AdminStatusBadge
                  tone={
                    selectedDocument.requiredForApproval ? "warning" : "neutral"
                  }
                >
                  {selectedDocument.requiredForApproval
                    ? t("settings.notice.required", "필수 동의")
                    : t("settings.notice.optional", "선택 문서")}
                </AdminStatusBadge>
                <AdminStatusBadge tone="neutral">
                  {selectedDocument.effectiveDateLabel}
                </AdminStatusBadge>
              </div>
              <p className="mt-3 text-sm leading-6 text-[var(--pbp-text-muted)]">
                {selectedDocument.summary}
              </p>
            </div>

            {selectedDocument.sourceNote ? (
              <p className="wafl-shape-control border border-[var(--pbp-border)] bg-[var(--pbp-surface)] px-4 py-3 text-xs font-semibold leading-5 text-[var(--pbp-text-muted)]">
                {selectedDocument.sourceNote}
              </p>
            ) : null}
            ...
            {selectedDocumentLoadState === "loading" ? (
              <WaflSettingCard
                title={t(
                  "settings.notice.documentLoadingTitle",
                  "문서를 불러오고 있습니다.",
                )}
                description={t(
                  "settings.notice.documentLoadingDescription",
                  "고객 공개 Markdown 원문을 읽어오는 중입니다.",
                )}
                tone="neutral"
                density="compact"
              />
            ...
            {selectedDocumentLoadState === "failed" ? (
              <WaflSettingCard
                title={t(
                  "settings.notice.documentFailedTitle",
                  "문서를 불러오지 못했습니다.",
                )}
                description={t(
                  "settings.notice.documentFailedDescription",
                  "정책 문서 원문을 읽을 수 없습니다. 문서 파일 위치를 확인해 주세요.",
                )}
                tone="warning"
                density="compact"
              />
            ...
            {selectedDocumentLoadState === "loaded" &&
            selectedDocumentContent ? (
              <article className="max-h-[60vh] overflow-auto wafl-shape-surface border border-[var(--pbp-border)] bg-[var(--pbp-surface)] p-6 [scrollbar-gutter:stable]">
                <div className="mb-5 wafl-shape-control border border-[var(--pbp-border)] bg-[var(--pbp-surface-soft)] px-4 py-3 text-xs font-semibold leading-5 text-[var(--pbp-text-muted)]">
                  원문 파일: {selectedDocumentContent.sourceFileName}
                </div>
                <MarkdownDocumentBody
                  markdown={selectedDocumentContent.markdown}
                />
              </article>
            ) : null}
          </div>
...
function FeedbackPanel() {
  ...
  const [feedbackNoticeTone, setFeedbackNoticeTone] =
    useState<ToastTone>("info");
    ...
    () => [
      {
        ...
        label: t("settings.feedback.type.feature", "기능 건의"),
      },
      {
        value: "bug" as const,
        label: t("settings.feedback.type.bug", "오류 제보"),
      },
      {
        value: "improvement" as const,
        label: t("settings.feedback.type.improvement", "개선 요청"),
      },
    ],
    [t],
  );
  ...
  const feedbackStatusTones: Record<
  ...
  const submitFeedback = useCallback(async () => {
    ...
    try {
      ...
      setFeedbackNotice(
        t(
          "settings.feedback.submitFailed",
  ...
  return (
    <WaflSectionPanel
      ...
      description={t(
        "settings.feedback.dbDescription",
        "기능 건의, 오류 제보, 개선 요청을 접수하고 처리 상태를 확인합니다.",
      )}
      actions={
        <AdminStatusBadge tone="info">
          {t("settings.feedback.dbMode", "문의 접수")}
        </AdminStatusBadge>
      }
      className="min-h-[320px]"
      headerClassName="min-h-[112px]"
    ...
    >
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.72fr)]">
        <WaflSettingsSectionGroup
          ...
          description={t(
            "settings.feedback.formDescriptionDb",
            "운영팀에 전달할 내용을 작성하면 접수 이력에 저장됩니다.",
          )}
          tone="neutral"
        >
          <div className="space-y-4">
            <div>
              <label
                className="text-xs font-bold text-[var(--pbp-text-subtle)]"
                htmlFor="admin-feedback-type"
              >
              ...
              <WaflSelect
                ...
                width="full"
                className="mt-2"
                ariaLabel={t("settings.feedback.typeLabel", "문의 유형")}
              />
            </div>
            ...
            <div>
              ...
              <WaflInput
                ...
                onChange={(event) => {
                  ...
                  if (feedbackSubmitState !== "idle")
                    setFeedbackSubmitState("idle");
                }}
                className="mt-2"
                placeholder={t(
                  "settings.feedback.titlePlaceholder",
                  "예: 사업자등록증 업로드 후 검토 상태가 궁금합니다",
                )}
              />
            </div>
            ...
            <div>
              <label
                className="text-xs font-bold text-[var(--pbp-text-subtle)]"
                htmlFor="admin-feedback-message"
              >
              ...
              <WaflTextarea
                ...
                placeholder={t(
                  ...
                  "불편한 상황, 재현 방법, 원하는 개선 방향을 적어 주세요.",
                )}
              />
              <p className="mt-2 text-xs leading-5 text-[var(--pbp-text-muted)]">
                {t(
                  "settings.feedback.validationHintDb",
                  "제목 2자 이상, 내용 10자 이상 입력하면 문의할 수 있습니다.",
                )}
              </p>
            </div>
            ...
            <div className="flex flex-wrap justify-end gap-2">
              ...
              >
                {feedbackSubmitState === "submitting"
                  ? t("common.saving", "저장 중")
                  : t("settings.feedback.submit", "문의하기")}
              </AdminButton>
            </div>
          </div>
        </WaflSettingsSectionGroup>
        ...
        <WaflSettingsSectionGroup
          ...
          description={t(
            "settings.feedback.historyDescriptionDb",
            "최근 접수한 문의의 처리 상태와 답변을 확인합니다.",
          )}
          badge={
            <AdminStatusBadge
              tone={feedbackLoadState === "failed" ? "warning" : "neutral"}
            >
              {feedbackLoadState === "loading"
                ? t("common.loadingShort", "조회 중")
                : t("settings.feedback.historyBadge", "최근 5건")}
            </AdminStatusBadge>
          }
          tone="neutral"
        >
          {feedbackLoadState === "failed" ? (
            <div className="wafl-shape-control border border-dashed border-[var(--pbp-border)] bg-[var(--pbp-surface-soft)] px-4 py-6 text-sm leading-6 text-[var(--pbp-text-muted)]">
              <p className="font-semibold text-[var(--pbp-text-primary)]">
                {t(
                  "settings.feedback.historyFailedTitle",
                  "접수 이력을 불러오지 못했습니다.",
                )}
              </p>
              <p className="mt-2">
                {t(
                  "settings.feedback.historyFailedDescription",
                  "문의 접수는 계속 사용할 수 있습니다. 이력은 잠시 후 다시 확인해 주세요.",
                )}
              </p>
            </div>
              ...
              {feedbackRequests.map((request) => {
                const typeLabel =
                  feedbackTypeOptions.find(
                    (option) => option.value === request.feedbackType,
                  )?.label ?? request.feedbackType;
                return (
                  <div
                    key={request.id}
                    className="wafl-shape-control border border-[var(--pbp-border)] bg-[var(--pbp-surface)] px-4 py-3 text-sm"
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="min-w-0">
                        <p className="truncate font-bold text-[var(--pbp-text-primary)]">
                          {request.title}
                        </p>
                        <p className="mt-1 text-xs text-[var(--pbp-text-muted)]">
                          {typeLabel} · {formatDateTime(request.createdAt)}
                        </p>
                      </div>
                      <AdminStatusBadge
                        tone={
                          feedbackStatusTones[request.feedbackStatus] ??
                          "neutral"
                        }
                        size="xs"
                      >
                        {feedbackStatusLabels[request.feedbackStatus] ??
                          request.feedbackStatus}
                      </AdminStatusBadge>
                    </div>
                    {request.responseMessage ? (
                      <p className="mt-3 wafl-shape-control bg-[var(--pbp-surface-soft)] px-3 py-2 text-xs leading-5 text-[var(--pbp-text-muted)]">
                        {request.responseMessage}
                      </p>
                    ) : null}
                  </div>
            ...
            <div className="wafl-shape-control border border-dashed border-[var(--pbp-border)] bg-[var(--pbp-surface-soft)] px-4 py-6 text-sm leading-6 text-[var(--pbp-text-muted)]">
              <p className="font-semibold text-[var(--pbp-text-primary)]">
                {t(
                  "settings.feedback.emptyHistoryTitle",
                  "아직 접수된 문의가 없습니다.",
                )}
              </p>
              <p className="mt-2">
                {t(
                  "settings.feedback.emptyHistoryDescriptionDb",
                  "문의가 접수되면 이 영역에 처리 상태가 표시됩니다.",
                )}
              </p>
            </div>
          )}
      ...
      <ToastMessage
        message={feedbackNotice || null}
        tone={feedbackNoticeTone}
        eventKey={feedbackNoticeEventKey}
      />
    </WaflSectionPanel>
  );
}
...
export default function AdminSettingsHub() {
  ...
  const [activeMenuId, setActiveMenuId] =
    useState<AdminSettingsMenuId>("account");
  const [billingPlanOverview, setBillingPlanOverview] =
    useState<AdminBillingPlanOverview | null>(null);
  const [companySubscription, setCompanySubscription] =
    useState<CompanySubscriptionSnapshot | null>(null);
  const [accountOverview, setAccountOverview] =
    useState<AdminAccountSettingsOverview | null>(null);
  const [accountRequests, setAccountRequests] = useState<
    CompanyAccountRequestRecord[]
  >([]);
  const [billingPlanLoadState, setBillingPlanLoadState] = useState<
    "idle" | "loading" | "loaded" | "failed"
  >("idle");
  const [subscriptionLoadState, setSubscriptionLoadState] = useState<
    "idle" | "loading" | "loaded" | "failed"
  >("idle");
  const [accountLoadState, setAccountLoadState] = useState<
    "idle" | "loading" | "loaded" | "failed"
  >("idle");
  const [accountRequestsLoadState, setAccountRequestsLoadState] = useState<
    "idle" | "loading" | "loaded" | "failed"
  >("idle");
    ...
    if (activeMenuId === "billing") {
      if (!billingPlanOverview) {
        return (
          <AdminEmptyState
            title={t(
              "settings.billing.emptyTitle",
              "요금제 정보를 불러오지 못했습니다.",
            )}
            description={t(
              "settings.billing.emptyDescription",
      ...
      return (
        <BillingPlanPanel
          overview={billingPlanOverview}
          loadState={billingPlanLoadState}
    ...
    if (activeMenuId === "account") {
      if (!accountOverview) {
        return (
          <AdminEmptyState
            title={t(
              "settings.account.emptyTitle",
              "계정 정보를 불러오지 못했습니다.",
            )}
            description={t(
              "settings.account.emptyDescription",
              "현재 로그인 회사와 관리자 계정 기준의 설정 데이터를 확인할 수 없습니다.",
            )}
          />
        );
      }
  ...
  const activeMenuItem =
    ADMIN_SETTINGS_MENU_ITEMS.find((item) => item.id === activeMenuId) ??
    ADMIN_SETTINGS_MENU_ITEMS[0];
  ...
  return (
    <div className="flex min-h-0 w-full flex-1 flex-col gap-3 overflow-y-auto sm:gap-4 md:gap-5">
      <WaflPageHero
        ...
        description={t(
          "settings.hub.description",
          "회사 정보와 운영 기준을 필요한 항목별로 관리합니다.",
        )}
        className="wafl-shape-surface p-4 sm:wafl-shape-surface sm:p-6"
        bodyClassName="mt-4 sm:mt-5"
      >
        <div className="mb-3 wafl-shape-control border border-[var(--pbp-border)] bg-[var(--pbp-surface)] px-3.5 py-3 shadow-sm md:hidden">
