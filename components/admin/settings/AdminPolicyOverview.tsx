  ...
  return (
    <AdminCard className="shrink-0 p-4">
      <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h2 className="text-lg font-semibold tracking-tight text-stone-950">{t("settings.policyOverview.title", "고객사 관리자 정책")}</h2>
        ...
        <span className="w-fit rounded-full bg-stone-950 px-3 py-1 text-xs font-semibold text-white">{t("settings.policyOverview.versionBadge", "0.9.216 기준")}</span>
      </div>

      <section className="mt-4">
        <div className="flex items-center justify-between gap-3">
          <h3 className="text-sm font-semibold text-stone-950">{t("settings.policyOverview.filePolicyTitle", "파일/용량 정책")}</h3>
          <span className="text-xs font-semibold text-stone-400">{t("settings.policyOverview.mainExposureLabel", "환경설정 메인 노출")}</span>
        </div>
      ...
      <section className="mt-5 grid gap-4 lg:grid-cols-[1fr_0.9fr]">
        <div className="rounded-3xl border border-stone-200 bg-stone-50 p-4">
          <h3 className="text-sm font-semibold text-stone-950">{t("settings.policyOverview.operationPolicyTitle", "운영 정책 정리")}</h3>
