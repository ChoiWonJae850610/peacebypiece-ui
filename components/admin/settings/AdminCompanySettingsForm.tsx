...
function SaveStateBadge({
  ...
  saveState: AdminSettingSaveState;
  labels: ReturnType<typeof useI18n>["i18n"]["admin"]["settingsForm"]["badges"];
}) {
  return (
  ...
  return (
    <AdminCard className="shrink-0 p-4">
      <div className="grid gap-3 lg:grid-cols-[1.35fr_0.85fr]">
        <section className="wafl-shape-surface bg-[var(--admin-theme-surface)] p-4 text-[var(--admin-theme-text-on-surface)] transition-colors">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-2xl font-semibold tracking-tight">
                {displayCompanyName}
              </h2>
              <p className="mt-2 text-xs font-semibold text-[var(--admin-theme-muted-on-surface)]">
                {text.planLabel}
              </p>
              <p className="mt-1 text-xs font-semibold text-[var(--admin-theme-muted-on-surface)]">
                {companyDate.updatedAt}
              </p>
            </div>
          ...
          <div className="mt-6 grid gap-2.5 sm:grid-cols-4">
            <div className="wafl-shape-control bg-[color:color-mix(in_srgb,var(--pbp-text-inverse)_10%,transparent)] px-3 py-3">
              <p className="text-[11px] font-semibold text-[var(--admin-theme-muted-on-surface)]">
                {text.summaryCards.status}
              </p>
              <p className="mt-2 text-sm font-semibold text-[var(--pbp-text-inverse)]">
                {text.summaryCards.statusValue}
              </p>
            </div>
            <div className="wafl-shape-control bg-[color:color-mix(in_srgb,var(--pbp-text-inverse)_10%,transparent)] px-3 py-3">
              <p className="text-[11px] font-semibold text-[var(--admin-theme-muted-on-surface)]">
                {text.summaryCards.joinedAt}
              </p>
              <p className="mt-2 text-sm font-semibold text-[var(--pbp-text-inverse)]">
                {companyDate.joinedAt}
              </p>
            </div>
            <div className="wafl-shape-control bg-[color:color-mix(in_srgb,var(--pbp-text-inverse)_10%,transparent)] px-3 py-3">
              <p className="text-[11px] font-semibold text-[var(--admin-theme-muted-on-surface)]">
                {text.summaryCards.age}
              </p>
              <p className="mt-2 text-sm font-semibold text-[var(--pbp-text-inverse)]">
                {companyDate.age}
              </p>
            ...
            <div className="wafl-shape-control bg-[color:color-mix(in_srgb,var(--pbp-text-inverse)_10%,transparent)] px-3 py-3">
              <p className="text-[11px] font-semibold text-[var(--admin-theme-muted-on-surface)]">
                {text.summaryCards.members}
              </p>
              <p className="mt-2 text-sm font-semibold text-[var(--pbp-text-inverse)]">
                {text.summaryCards.memberValue}
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
          <WaflSurface shape="control" tone="muted" className="p-3.5">
            ...
            <div className="mt-3 grid grid-cols-4 gap-2">
              {ADMIN_THEME_OPTIONS.map((option) => (
                <button
                  ...
                  onClick={() =>
                    saveNextSettings(
                      withAdminSettingsUiDraft(draft, {
