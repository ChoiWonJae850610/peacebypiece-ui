...
export default function AdminShell({ companyName, appVersion, title, description, children, contentMode = "scroll" }: AdminShellProps) {
  ...
  const contentFrameClassName = isFixedFromDesktop
    ? "min-h-0 flex-1 overflow-y-auto overscroll-contain pr-1 md:overflow-hidden"
    : "min-h-0 flex-1 overflow-y-auto overscroll-contain pr-1";
  const contentInnerClassName = isFixedFromDesktop
    ? "flex min-h-full flex-col gap-5 pb-2 md:h-full md:min-h-0 md:gap-0 md:pb-0"
    : "flex min-h-full flex-col gap-5 pb-2";
