...
import {
  ADMIN_REGRESSION_POLICY_NOTES,
  ADMIN_REGRESSION_ROUTES,
  type AdminRegressionRouteItem,
  type AdminRegressionRouteStatus,
} from "@/lib/admin/adminRegressionRoutes";

interface AdminRegressionRoutePageProps {
  title: string;
  eyebrow: string;
  description: string;
...
function getStatusClassName(status: AdminRegressionRouteStatus) {
  if (status === "stable") {
...
function AdminRouteCard({ route }: { route: AdminRegressionRouteItem }) {
...
export default function AdminRegressionRoutePage({
  title,
  eyebrow,
  description,
  currentRouteId,
}: AdminRegressionRoutePageProps) {
  const currentRoute = ADMIN_REGRESSION_ROUTES.find(
    ...
    <main className="min-h-screen bg-stone-50 px-4 py-6 text-stone-900 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {ADMIN_REGRESSION_ROUTES.map((route) => (
            <AdminRouteCard key={route.id} route={route} />
          ))}
        </section>
        ...
        <section className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm">
          ...
          <ul className="mt-4 grid gap-3 lg:grid-cols-4">
            {ADMIN_REGRESSION_POLICY_NOTES.map((note) => (
              <li
                key={note}
