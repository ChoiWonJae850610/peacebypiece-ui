import type { AdminManagedFileItem } from "@/lib/admin/adminFiles.presentation";

type FileListSectionProps = {
  items: AdminManagedFileItem[];
};

export default function FileListSection({ items }: FileListSectionProps) {
  return (
    <section className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 overflow-hidden rounded-2xl border border-stone-200">
        ...
        <div className="divide-y divide-stone-200">
          {items.map((item) => (
            <article key={item.id} className="grid gap-3 px-4 py-4 text-sm md:grid-cols-[1.4fr_1.4fr_0.7fr_0.7fr_0.8fr_0.8fr] md:items-center">
              <div>
