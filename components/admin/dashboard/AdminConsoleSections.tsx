...
function AdminWorkspaceCardView({ item }: { item: AdminWorkspaceCard }) {
  ...
  const content = (
    <AdminCard
      as="article"
      className="flex h-full min-h-[150px] p-5 transition hover:-translate-y-0.5 hover:border-stone-300 hover:shadow-md sm:min-h-[162px] sm:p-6 lg:min-h-[174px]"
    >
      <div className="flex h-full min-w-0 flex-1 flex-col justify-between gap-4">
        <div className="min-w-0">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <h2 className="text-lg font-semibold tracking-tight text-stone-950">{text.label}</h2>
