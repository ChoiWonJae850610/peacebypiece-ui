      ...
      <div className="flex min-w-0 flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          ...
          <div className="mt-1.5 flex min-w-0 flex-wrap items-center gap-2">
            <h3 className="text-sm font-black tracking-[-0.03em] text-[var(--pbp-text-primary)] sm:text-base">{title}</h3>
            {badge ? <span className="shrink-0">{badge}</span> : null}
          </div>
          {description ? <p className="mt-2 max-w-3xl text-xs leading-5 text-[var(--pbp-text-muted)] sm:text-sm sm:leading-6">{description}</p> : null}
        </div>
        {aside ? <div className="flex w-full shrink-0 flex-wrap items-center gap-2 lg:w-auto lg:justify-end">{aside}</div> : null}
      </div>
