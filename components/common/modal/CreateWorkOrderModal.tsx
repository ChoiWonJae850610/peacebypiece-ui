...
import {
  ...
  DEFAULT_CATEGORY2,
  DEFAULT_CATEGORY3,
} from "@/lib/constants/workorderCategories";
...
import { WaflInput } from "@/components/common/ui";
...
export default function CreateWorkOrderModal({
  ...
  const [categorySource, setCategorySource] = useState<CategorySource>(() =>
    buildDefaultCategorySource(),
  );
  const [category1, setCategory1] = useState<string>(DEFAULT_CATEGORY1);
  const category2Options = useMemo(
    () =>
      categorySource.category2OptionsMap[category1] ?? [
        createFallbackOption(i18n.common.ui.common.uncategorized),
      ],
    [
      categorySource.category2OptionsMap,
      category1,
      i18n.common.ui.common.uncategorized,
    ],
  );
  const [category2, setCategory2] = useState<string>(
    category2Options[0]?.name ?? DEFAULT_CATEGORY2,
  );
  const category3Options = useMemo(
    () =>
      categorySource.category3OptionsMap[category2] ?? [
        createFallbackOption(i18n.common.ui.common.uncategorized),
      ],
    [
      categorySource.category3OptionsMap,
      category2,
      i18n.common.ui.common.uncategorized,
    ],
  );
  const [category3, setCategory3] = useState<string>(
    category3Options[0]?.name ?? DEFAULT_CATEGORY3,
  );
  ...
  useEffect(() => {
    ...
    const nextCategory2 =
      categorySource.category2OptionsMap[nextCategory1.name]?.[0] ??
      categorySource.defaultCategory2;
    const nextCategory3 =
      categorySource.category3OptionsMap[nextCategory2.name]?.[0] ??
      categorySource.defaultCategory3;
  ...
  useEffect(() => {
    ...
    fetchAdminStandardsFromApi()
      .then((payload) => {
        if (!isMounted) return;
        setCategorySource(
          buildCategorySourceFromDefinitions(payload.itemCategories),
        );
      })
      .catch(() => undefined);
    return () => {
  ...
  useEffect(() => {
    const nextCategory2 =
      categorySource.category2OptionsMap[category1]?.[0] ??
      categorySource.defaultCategory2;
    setCategory2(nextCategory2.name);
  ...
  useEffect(() => {
    const nextCategory3 =
      categorySource.category3OptionsMap[category2]?.[0] ??
      categorySource.defaultCategory3;
    setCategory3(nextCategory3.name);
  }, [categorySource, category2]);

  const trimmedTitle = title.trim();
  const submitDisabled = getModalActionDisabledState(
    trimmedTitle.length === 0 || isCreating,
  );
  const recommendedCategory = useMemo(
    () =>
      WORKORDER_CATEGORY_RECOMMENDATION_ENABLED
        ? getRecommendedWorkOrderCategory(trimmedTitle)
        : null,
    [trimmedTitle],
  );

  const selectedCategory1 = findCategoryOption(
    categorySource.category1Options,
    category1,
    categorySource.defaultCategory1,
  );
  const selectedCategory2 = findCategoryOption(
    category2Options,
    category2,
    categorySource.defaultCategory2,
  );
  const selectedCategory3 = findCategoryOption(
    category3Options,
    category3,
    categorySource.defaultCategory3,
  );
  ...
  return (
    <ModalShell
      ...
      footer={renderModalFooterActions({
        layout: "end",
        secondary: {
          label: i18n.common.ui.common.cancel ?? MODAL_ACTION_LABELS.cancel,
          onClick: onClose,
          disabled: isCreating,
        },
        primary: {
          label: isCreating
            ? copy.creating
            : (i18n.common.ui.common.create ?? MODAL_ACTION_LABELS.create),
    ...
    >
      <div className="grid gap-4">
        <label className="grid gap-1.5">
          <span className="text-sm font-medium text-[var(--pbp-text-secondary)]">
            {copy.workOrderTitle}
          </span>
          <WaflInput
            value={title}
        ...
        {isCreating ? (
          <div
            className={`${MODAL_CONTENT_MUTED_PANEL_CLASS} flex items-center gap-2 px-3 py-2 text-sm text-[var(--pbp-text-secondary)]`}
          >
            <span
              className="h-4 w-4 animate-spin rounded-full border-2 border-[var(--pbp-border)] border-t-[var(--pbp-text-secondary)]"
              aria-hidden="true"
            />
        ...
        <CreateWorkOrderCategoryFields
          ...
          options={{
            category1Options: categorySource.category1Options,
            category2Options,
            category3Options,
          }}
          onChange={{
