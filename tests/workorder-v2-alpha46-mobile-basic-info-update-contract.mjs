...
for (const label of ["제품명", "납기", "총 수량", "취소", "완료", "저장 중"]) {
  assert.match(`${app}\n${detail}`, new RegExp(label), `mobile basic-info UI missing ${label}`);
}
assert.doesNotMatch(app, /setSaveMessage(?:State)?\("저장됨"\)/);
...
assert.match(app, /(?:const|let) saved = await workOrderMutationController\.updateOverview/);
...
assert.doesNotMatch(saveBasicInfo, /workOrderQueryController\.detail/);
assert.match(app, /setItems\(\(current\) => current\.map/);
assert.doesNotMatch(saveBasicInfo, /getWorkOrderList\(/);
