...
assert.match(experience, /materialType: activeMaterialType/);
...
assert.match(experience, /materialCacheKey\(detail\.header\.id, activeMaterialType\)/);
assert.doesNotMatch(experience, /materialType: "fabric"/);

assert.match(overview, /tab\.id === "fabric" \|\| tab\.id === "accessory"/);
assert.match(overview, /setActiveSection\(tab\.id\)/);
assert.match(overview, /props\.onOpenMaterials\(tab\.id\)/);
assert.match(overview, /materialType=\{props\.materialType\}/);
...
assert.match(commandService, /configuredApproval === WAFL_V2_ALPHA56_ACCESSORY_LIFECYCLE_PARITY_MUTATION_APPROVAL/);
assert.match(externalQa, /alpha56AccessoryLifecycleParityEnabled/);
