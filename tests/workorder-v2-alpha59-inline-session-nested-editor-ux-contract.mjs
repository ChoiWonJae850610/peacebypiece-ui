...
const existingEditor = structure.slice(structure.indexOf("function ExistingStructureEditor"), structure.indexOf("function StructureCard"));
assert.doesNotMatch(existingEditor, /if \(saved\)[\s\S]{0,100}props\.onClose\(\)/);
assert.match(existingEditor, /const cancelPalette =/);
assert.match(existingEditor, /const applyPalette = async/);
assert.match(existingEditor, /onCancel=\{cancelPalette\}/);
assert.match(existingEditor, /onConfirm=\{applyPalette\}/);
assert.match(existingEditor, /selectedColor\.id/);
assert.match(existingEditor, /selectedValue=\{selectedId\}/);
assert.equal((existingEditor.match(/props\.edit\.onPatchColor/g) ?? []).length, 2);
