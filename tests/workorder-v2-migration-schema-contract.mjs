...
if (/APP_VERSION = "2\.0\.0-alpha\.(44|45)"/.test(appVersion)) {
  ...
  assert.deepEqual(
    ...
    [],
    "alpha.40 may add only the authenticated generated-document file route under app/api",
  );
  ...
  assert.deepEqual(
    ...
    [],
    "alpha.30 preparation may add only the exact process PATCH route",
  );
} else if (appVersion.includes('APP_VERSION = "2.0.0-alpha.29"')) {
  assert.deepEqual(
    apiChanges.filter((change) => ![...alpha25ApiPaths, ...alpha26ApiPaths, ...alpha27ApiPaths, ...alpha28ApiPaths, ...alpha29ApiPaths].some((allowedPath) => change.endsWith(allowedPath))),
    [],
    "alpha.29 may add only the exact document-number Preview resolver GET route",
  );
  ...
  assert.deepEqual(
    apiChanges.filter((change) => ![...alpha25ApiPaths, ...alpha26ApiPaths, ...alpha27ApiPaths, ...alpha28ApiPaths].some((allowedPath) => change.endsWith(allowedPath))),
    [],
    "alpha.28 may add only the exact issued Preview GET route beside existing WorkOrder Command routes",
  );
