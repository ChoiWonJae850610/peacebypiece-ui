...
assert.match(processRepository,/expectedVersion/);assert.match(processRepository,/work_order_status!=="draft"/);assert.match(processRepository,/revision_status!=="draft"/);assert.match(processRepository,/WORK_ORDER_COMMAND_CODES\.productionProcess\.update/);assert.match(commandCodes,/update: "work_order\.production_process\.update"/);assert.doesNotMatch(processRepository,/application_area\s*=|application_color_target\s*=/,"hidden alpha.30 snapshots must remain untouched");
