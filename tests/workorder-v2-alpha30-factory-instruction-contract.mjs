...
for(const text of ["작업지시서","개정차수","제품 스케치","공장 전달 메모","사용 부위","색상·사이즈 수량","사이즈 스펙","제작 공정·추가 공정"])assert.ok(preview.includes(text),`Preview missing: ${text}`);
...
for(const forbidden of ["재고","발주수량</th>","단가","금액","\"Revision\"",">Revision<","보내는 시점",...(alpha42ContractExists?[]:["QR"])])assert.doesNotMatch(preview,new RegExp(forbidden));
assert.match(css,/@page\s+cover\s*\{\s*size:\s*A4 landscape/);assert.match(css,/@page\s+content\s*\{\s*size:\s*A4 portrait/);assert.match(css,/break-after:\s*page/);assert.match(css,/thead\s*\{\s*display:\s*table-header-group/);assert.match(css,/@media print/);
