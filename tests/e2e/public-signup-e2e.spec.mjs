...
test.describe("public signup QA smoke", () => {
  test("login CTA opens the public Trial signup page", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "Google로 가입 신청 시작" })).toHaveAttribute(
      "href",
  ...
  test("public signup and invitation entry points stay separated", async ({ page }) => {
    ...
    await expect(page.getByText("공개 가입은 신규 회사 Trial 신청 전용입니다.")).toBeVisible();
    await expect(page.getByRole("link", { name: "로그인" })).toHaveAttribute("href", "/login");
    await expect(page.getByText(/카드 번호는 WAFL에 저장하지 않습니다/)).toBeVisible();
    await expect(page.getByText(/지금 결제되는 금액은 0원/)).toBeVisible();
  });
});
