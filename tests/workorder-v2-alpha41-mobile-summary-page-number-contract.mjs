...
assert.equal(fs.readdirSync(path.join(root, "db/v2/migrations")).filter((name) => /^\d{3}_.*\.sql$/.test(name)).length, fs.existsSync(path.join(root, "db/v2/migrations/020_v2_sample_reorder_invariant.sql")) ? 20 : fs.existsSync(path.join(root, "db/v2/migrations/019_v2_work_order_lineage_sample.sql")) ? 19 : fs.existsSync(path.join(root, "db/v2/migrations/018_v2_company_spec_item_category_scope.sql")) ? 18 : fs.existsSync(path.join(root, "db/v2/migrations/017_v2_company_spec_item_catalog.sql")) ? 17 : fs.existsSync(path.join(root, "db/v2/migrations/016_v2_r0_document_snapshot_and_managed_qr.sql")) ? 16 : fs.existsSync(path.join(root, "db/v2/migrations/015_v2_company_work_order_structure_options.sql")) ? 15 : fs.existsSync(path.join(root, "db/v2/migrations/014_v2_size_spec_templates.sql")) ? 14 : fs.existsSync(path.join(root, "db/v2/migrations/013_v2_material_line_archive_lifecycle.sql")) ? 13 : alpha42ContractExists ? 12 : 11);
