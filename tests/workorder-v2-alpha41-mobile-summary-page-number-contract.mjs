...
assert.match(documentRenderer, /function DocumentFooter/);
assert.match(documentRenderer, /data-testid="workorder-page-number"/);
assert.match(documentRenderer, /\{pageNumber\} \/ \{totalPages\}/);
assert.match(documentRenderer, /DocumentFooter data=\{data\} pageNumber=\{1\} totalPages=\{totalPages\}/);
assert.match(documentRenderer, /DocumentFooter data=\{data\} pageNumber=\{pageIndex \+ 2\} totalPages=\{totalPages\}/);
assert.match(css, /\.page \{ position: relative;/);
assert.match(css, /\.pageNumberFooter \{[\s\S]*?position: absolute;[\s\S]*?left: 12mm;[\s\S]*?right: 12mm;[\s\S]*?bottom: 8mm;[\s\S]*?grid-template-columns:/);
assert.match(css, /@media \(max-width: 760px\)[\s\S]*?\.page \{ width: 100%; min-height: 0; padding: 18px 12px 36px;/);
...
assert.match(pageOrientation, /const expectedOrientation = "portrait"/);
...
assert.equal(fs.readdirSync(path.join(root, "db/v2/migrations")).filter((name) => /^\d{3}_.*\.sql$/.test(name)).length, fs.existsSync(path.join(root, "db/v2/migrations/022_v2_work_order_drawings.sql")) ? 22 : fs.existsSync(path.join(root, "db/v2/migrations/021_v2_work_order_image_output_include.sql")) ? 21 : fs.existsSync(path.join(root, "db/v2/migrations/020_v2_sample_reorder_invariant.sql")) ? 20 : fs.existsSync(path.join(root, "db/v2/migrations/019_v2_work_order_lineage_sample.sql")) ? 19 : fs.existsSync(path.join(root, "db/v2/migrations/018_v2_company_spec_item_category_scope.sql")) ? 18 : fs.existsSync(path.join(root, "db/v2/migrations/017_v2_company_spec_item_catalog.sql")) ? 17 : fs.existsSync(path.join(root, "db/v2/migrations/016_v2_r0_document_snapshot_and_managed_qr.sql")) ? 16 : fs.existsSync(path.join(root, "db/v2/migrations/015_v2_company_work_order_structure_options.sql")) ? 15 : fs.existsSync(path.join(root, "db/v2/migrations/014_v2_size_spec_templates.sql")) ? 14 : fs.existsSync(path.join(root, "db/v2/migrations/013_v2_material_line_archive_lifecycle.sql")) ? 13 : alpha42ContractExists ? 12 : 11);
