...
for (const token of [
  ...
  "expectedProvisioningStatus",
  "compareAndSet: true",
]) {
  assert.ok(`${service}\n${repository}`.includes(token), `failure contract missing ${token}`);
}

assert.match(service, /throw new SignupProvisioningPersistedError\(input\.application\.id, errorCode\)/);
assert.match(service, /catch \{\s*throw new SignupProvisioningPersistedError\(/);
