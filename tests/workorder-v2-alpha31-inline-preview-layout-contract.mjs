...
for (const name of ["InlineEditableValue", "ExpandableInlineNote", "ReadOnlyInlineValue"]) assert.match(inline, new RegExp(`export function ${name}`));
assert.match(inline, /borderStyle: "dotted"/);
assert.match(inline, /fontSize: 16/);
assert.match(inline, /numberOfLines=\{2\}/);
...
assert.match(mobile, /<InlineEditableValue[\s\S]*label="적용 부위"/);
assert.match(mobile, /<InlineEditableValue[\s\S]*label="적용 색상·대상"/);
assert.match(mobile, /<ExpandableInlineNote[\s\S]*label="작업 메모"/);
...
for (const token of ["리넨 라운드 원피스", "144", "IVORY", "NAVY", "BLACK", "자개 단추", "콘실 지퍼", "케어 라벨", "폴리백", "재단", "봉제", "다림질", "검수·포장"]) assert.ok(sample.includes(token), `sample missing ${token}`);
