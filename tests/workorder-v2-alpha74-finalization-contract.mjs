...
assert.equal(version, "2.0.0-alpha.74");
...
assert.match(expoEnvironment, /Internal APP_VERSION \| `2\.0\.0-alpha\.74`/u);
...
assert.match(sketchEditor, /type SketchTool = "pen" \| "line" \| "arrow" \| "rectangle" \| "ellipse" \| "text"/u);
for (const label of ["펜", "선", "화살표", "사각형", "타원", "텍스트"]) {
  assert.match(sketchEditor, new RegExp(`label="${label}"`, "u"));
}
for (const deferredLabel of ["지우개", "선택", "이동", "확대", "이미지", "연필"]) {
