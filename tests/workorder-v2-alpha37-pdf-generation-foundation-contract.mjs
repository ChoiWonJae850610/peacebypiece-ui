...
assert.match(version, /2\.0\.0-alpha\.37/);
assert.match(mobileVersion, /2\.0\.0-alpha\.37/);
assert.equal(JSON.parse(appJson).expo.version, "2.0.0-alpha.37");
assert.equal(JSON.parse(appJson).expo.extra.appVersion, "2.0.0-alpha.37");
assert.equal(JSON.parse(mobilePackage).version, "2.0.0-alpha.37");
assert.equal(JSON.parse(mobileLock).version, "2.0.0-alpha.37");
assert.equal(JSON.parse(mobileLock).packages[""].version, "2.0.0-alpha.37");

assert.match(constants, /WORK_ORDER_PDF_RENDERER_VERSION\s*=\s*"wafl-work-instruction-pdf\/1"/);
assert.match(constants, /WORK_ORDER_PDF_DTO_SCHEMA_VERSION\s*=\s*1/);
assert.match(constants, /WORK_ORDER_PDF_DOCUMENT_TYPE\s*=\s*"factory_instruction"/);
assert.match(constants, /10 \* 1024 \* 1024/);
