...
assert.match(optionReel, /selectedIndexRef/);
assert.match(optionReel, /\}, \[options\]\);/);
assert.doesNotMatch(optionReel, /\}, \[options, selectedIndex\]\);/);
const commitScrollIndex = optionReel.match(/function commitScrollIndex\([\s\S]*?\n  \}/)?.[0] ?? "";
...
assert.match(optionReel, /onMomentumScrollEnd=\{commitScrollIndex\}/);
assert.match(optionReel, /onScrollEndDrag=\{\(event\) => \{[\s\S]*?commitScrollIndex\(event\)/);
assert.match(optionReel, /snapToAlignment="start"/);
assert.match(optionReel, /snapToInterval=\{ITEM_HEIGHT\}/);
assert.match(optionReel, /extraData=\{visibleSelectedIndex\}/);
