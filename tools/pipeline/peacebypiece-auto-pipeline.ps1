...
function GetDbMigrationAppliedDisplayValue {
    ...
    if ([string]$VerificationSummary.DbMigrationApplyResult -match '^PASS - approved dev/test migration 012') {
        return "true; approved dev/test migration 012 applied once"
    }
...
function GetDbSchemaMutationDisplayValue {
    ...
    if ([string]$VerificationSummary.SchemaMigrationThisRun -match '^012_v2_document_access_token_purpose\.sql') {
        return "true; additive token-purpose column, CHECK, and partial unique index only on approved dev/test DB"
    }
...
function GetLocalRepoVerificationSummary {
    ...
    $alpha40Completion = $version -eq "2.0.0-alpha.40"
    $alpha41Completion = $version -eq "2.0.0-alpha.41"
    $alpha42Completion = $version -eq "2.0.0-alpha.42"
    ...
    return [pscustomobject]@{
        ...
        PdfR2Regression = if ($alpha42Completion) { "PASS - Worker-mediated PDF PUT/GET/DELETE 1/3/0; retained size/SHA/pages/orientation verified" } elseif ([string]::IsNullOrWhiteSpace($pdfR2Regression)) { if ($productUxCleanupProfile) { "PASS - workorder PDF route/viewer static contracts; live R2 NOT_RUN" } elseif ($workorderPdfLiveProfile) { "PASS - live dev/test PDF/R2 lifecycle integration and static contracts" } elseif ($productUiRuntimeProfile) { "PASS - dev/test workorder PDF create/view/render evidence; user visual review remains" } else { "not provided" } } else { $pdfR2Regression }
        ...
        ManualQaStatus = if ($alpha42Completion) { "PENDING_USER_QA - physical QR scan, printed QR readability, and final document visual judgment" } elseif ([string]::IsNullOrWhiteSpace($manualQaStatus)) { if ($productUxCleanupProfile) { "PENDING_USER_QA - PDF visual/browser matrix" } elseif ($workorderPdfLiveProfile) { "PENDING_USER_QA - rendered PDF visual/browser/device confirmation" } elseif ($productUiRuntimeProfile) { "PRODUCT_QA_INCOMPLETE - user PDF/screen review and same-Google OAuth return remain" } else { "not provided" } } else { $manualQaStatus }
        DbMigrationApplyResult = if ($alpha42Completion) { "PASS - approved dev/test migration 012 applied once; ledger 12/12" } elseif ($alpha40Completion -or $alpha41Completion) { "NOT_APPLICABLE - no migration in alpha.40/41; retained ledger 11/11" } elseif ($alpha39Completion -and $alpha39MigrationPassed) { "PASS - approved dev/test migration 011 applied once; ledger 11/11" } elseif ($alpha30Completion) { "PASS - approved dev/test migration 009 applied once; ledger 9/9" } elseif ($productUxCleanupProfile) { "NOT_APPLICABLE - no migration in 0.24.34.2" } elseif ($workorderPdfLiveProfile) { "NOT_APPLICABLE - no migration in 0.24.34.3" } elseif ($productUiRuntimeProfile) { "NOT_APPLICABLE - no migration in 0.24.34.5" } elseif ($null -ne $workorderEvidence -and $workorderEvidence.DbMigrationApplyResult -ne "not provided") { $workorderEvidence.DbMigrationApplyResult } elseif ($null -ne $billingEvidence -and $billingEvidence.DbMigrationApplyResult -ne "not provided") { $billingEvidence.DbMigrationApplyResult } else { GetLocalRepoVerificationEvidenceLine -Lines $lines -Patterns @(
        ...
        PostApplyAuditResult = if ($alpha42Completion) { "PASS - ledger 12/12; token purpose/CHECK/index present; existing manual-share rows unchanged" } elseif ($alpha40Completion -or $alpha41Completion) { "NOT_APPLICABLE - no migration in alpha.40/41" } elseif ($alpha30Completion) { "PASS - four nullable fields and NOT VALID constraints; existing rows/RLS/ACL unchanged" } elseif ($productUxCleanupProfile) { "NOT_APPLICABLE - no migration in 0.24.34.2" } elseif ($workorderPdfLiveProfile) { "NOT_APPLICABLE - no migration in 0.24.34.3" } elseif ($productUiRuntimeProfile) { "NOT_APPLICABLE - no migration in 0.24.34.5" } elseif ($null -ne $workorderEvidence -and $workorderEvidence.PostApplyAuditResult -ne "not provided") { $workorderEvidence.PostApplyAuditResult } elseif ($null -ne $billingEvidence -and $billingEvidence.PostApplyAuditResult -ne "not provided") { $billingEvidence.PostApplyAuditResult } else { GetLocalRepoVerificationEvidenceLine -Lines $lines -Patterns @(
            'Post-Apply Audit Result:\s*(PASS|FAIL|NOT_APPLICABLE)',
            'Post-apply .*audit:\s*(PASS|FAIL)',
            'post-apply .*findings\s+0'
        ) }
        RollbackSmokeResult = if ($alpha42Completion) { "NOT_RUN - successful dev/test evidence retained; cleanup/reset/rollback/delete 0" } elseif ($alpha40Completion -or $alpha41Completion) { "NOT_APPLICABLE - no migration or data mutation; cleanup/reset/rollback not executed" } elseif ($alpha30Completion) { "NOT_RUN - cleanup/reset/rollback prohibited and not executed" } elseif ($productUxCleanupProfile) { "NOT_APPLICABLE - no DB fixture mutation" } elseif ($workorderPdfLiveProfile) { "PASS - exact fixture cleanup with residual DB/R2 0" } elseif ($productUiRuntimeProfile) { "NOT_APPLICABLE - no cleanup fixture; dev/test existing workorder QA mutation recorded" } elseif ($null -ne $billingEvidence -and $billingEvidence.RollbackSmokeResult -ne "not provided") { $billingEvidence.RollbackSmokeResult } else { GetLocalRepoVerificationEvidenceLine -Lines $lines -Patterns @(
            'Rollback Smoke Result:\s*(PASS|FAIL)',
            'rollback smoke:\s*(PASS|FAIL)',
            'rollback smoke .*PASS'
        ) }
        DevTestDbTestDataMutation = if ($alpha42Completion) { "true - approved retained realistic Company A issued-document evidence" } elseif ($alpha30Completion) { "true - approved Company A synthetic WorkOrder/revision/material Command evidence" } else { GetLocalRepoVerificationNamedValue -Lines $lines -Name "Dev/Test DB Test-Data Mutation" -Fallback "false" }
        DevTestFixtureMutation = if ($alpha42Completion) { "true - approved realistic content and representative-image linkage fixture" } elseif ($alpha30Completion) { "true - one approved direct process fixture row" } elseif ($null -ne $workorderEvidence -and $workorderEvidence.DevTestFixtureMutation -ne "not provided") { $workorderEvidence.DevTestFixtureMutation } elseif ($null -ne $billingEvidence -and $billingEvidence.DevTestFixtureMutation -ne "not provided") { $billingEvidence.DevTestFixtureMutation } else { GetLocalRepoVerificationNamedValue -Lines $lines -Name "Dev/Test Fixture Mutation" -Fallback "false" }
        ...
        DevTestR2Mutation = if ($alpha42Completion) { "true - Cloudflare Worker-mediated representative image PUT 1 and PDF PUT 1 retained; PDF GET 3; DELETE 0" } elseif ($alpha40Completion -or $alpha41Completion) { "false in alpha.40/41; R2 GET 2 only; retained alpha.38 object unchanged" } elseif ($alpha39Completion -and $alpha39RuntimePassed) { "false in alpha.39; historical alpha.38 approved dev/test PUT 1 retained" } elseif ($workorderPdfLiveProfile) { "true - synthetic PDF/R2 fixture integration only" } elseif ($null -ne $workorderEvidence -and $workorderEvidence.DevTestR2Mutation -ne "not provided") { $workorderEvidence.DevTestR2Mutation } elseif ($null -ne $billingEvidence -and $billingEvidence.DevTestR2Mutation -ne "not provided") { $billingEvidence.DevTestR2Mutation } else { GetLocalRepoVerificationNamedValue -Lines $lines -Name "Dev/Test R2 Mutation" -Fallback "false" }
        ProductionMutation = if ($alpha42Completion) { "false" } elseif ($workorderPdfLiveProfile) { "false" } elseif ($null -ne $workorderEvidence -and $workorderEvidence.ProductionMutation -ne "not provided") { $workorderEvidence.ProductionMutation } elseif ($null -ne $billingEvidence -and $billingEvidence.ProductionMutation -ne "not provided") { $billingEvidence.ProductionMutation } else { GetLocalRepoVerificationNamedValue -Lines $lines -Name "Production Mutation" -Fallback "false" }
        SchemaMigrationThisRun = if ($alpha42Completion) { "012_v2_document_access_token_purpose.sql; token_purpose column + CHECK + partial unique index; ledger 12/12" } elseif ($alpha40Completion -or $alpha41Completion) { "false - no migration/schema/index change; retained ledger 11/11" } elseif ($alpha39Completion -and $alpha39MigrationPassed) { "011_v2_document_access_viewer_functions.sql; functions +2; table/column/index/data delta 0" } elseif ($alpha30Completion) { "false - migration 009 was applied in the separately approved pre-runtime phase" } elseif ($workorderPdfLiveProfile) { "false" } elseif ($null -ne $workorderEvidence -and $workorderEvidence.SchemaMigrationThisRun -ne "not provided") { $workorderEvidence.SchemaMigrationThisRun } elseif ($null -ne $billingEvidence -and $billingEvidence.SchemaMigrationThisRun -ne "not provided") { $billingEvidence.SchemaMigrationThisRun } else { GetLocalRepoVerificationNamedValue -Lines $lines -Name "Schema Migration This Run" -Fallback "false" }
        ...
        FinalResidualDbRows = if ($alpha42Completion) { "retained approved dev/test realistic WorkOrder/revision/content/image, generated document, embedded token, receipts, and events" } elseif ($productUxCleanupProfile) { "NOT_APPLICABLE - no DB mutation" } elseif ($workorderPdfLiveProfile) { "0" } elseif ($productUiRuntimeProfile) { "NOT_APPLICABLE - existing dev/test workorder mutation, no cleanup fixture" } elseif ($null -ne $workorderEvidence -and $workorderEvidence.FinalResidualDbRows -ne "not provided") { $workorderEvidence.FinalResidualDbRows } elseif ($null -ne $billingEvidence -and $billingEvidence.FinalResidualDbRows -ne "not provided") { $billingEvidence.FinalResidualDbRows } else { GetLocalRepoVerificationNamedValue -Lines $lines -Name "Final Residual DB Rows" }
        FinalResidualR2Objects = if ($alpha42Completion) { "retained representative PNG and immutable PDF; no delete" } elseif ($productUxCleanupProfile) { "NOT_APPLICABLE - no R2 mutation" } elseif ($workorderPdfLiveProfile) { "0" } elseif ($productUiRuntimeProfile) { "NOT_APPLICABLE - workorder PDF evidence generated in dev/test" } elseif ($null -ne $workorderEvidence -and $workorderEvidence.FinalResidualR2Objects -ne "not provided") { $workorderEvidence.FinalResidualR2Objects } elseif ($null -ne $billingEvidence -and $billingEvidence.FinalResidualR2Objects -ne "not provided") { $billingEvidence.FinalResidualR2Objects } else { GetLocalRepoVerificationNamedValue -Lines $lines -Name "Final Residual R2 Objects" }
        LiveViewerIntegration = if ($alpha42Completion) { "PASS - exchange 1, inline/download 2, access count 1, pdf.share_viewed +1; live H/C not executed" } elseif ($productUxCleanupProfile) { "NOT_RUN - static viewer fallback contract only" } elseif ($workorderPdfLiveProfile) { "PASS - server proxy viewer path and live R2 object read-back" } else { GetLocalRepoVerificationNamedValue -Lines $lines -Name "Live Viewer Integration" }
    }
}
...
function AddAlpha41MobileSummaryPageNumberRepoStateSections {
    param(
        [System.Collections.Generic.List[string]]$Lines,
        [string]$Version
    )

    if ($Version -ne "2.0.0-alpha.41") { return }

    $manifest = $null
    $manifestPath = Join-Path $ProjectDir ".tmp\wafl-v2-alpha41\product-qa-manifest.json"
    if (Test-Path -LiteralPath $manifestPath -PathType Leaf) {
        try { $manifest = Get-Content -LiteralPath $manifestPath -Raw -Encoding UTF8 | ConvertFrom-Json } catch { $manifest = $null }
    }
    $passed = $null -ne $manifest -and [string]$manifest.result -eq "LEVEL_4_PRODUCT_VERIFIED"
    $pdf = if ($passed) { $manifest.samplePdf } else { $null }
    $actual = if ($passed) { $manifest.actualPdfRegression } else { $null }

    AddRepoStateSection -Lines $Lines -Title "Alpha.41 Completion Status:" -Values @($(if ($passed) { "LEVEL_4_PRODUCT_VERIFIED" } else { "MISSING_OR_INVALID_PRODUCT_QA_MANIFEST" }))
    AddRepoStateSection -Lines $Lines -Title "Alpha.41 Phone Breakpoint / Summary Lines:" -Values @($(if ($passed) { "$($manifest.phoneBreakpoint) / 2" } else { "760 / unknown" }))
    AddRepoStateSection -Lines $Lines -Title "Alpha.41 320 / 375 / 425 Summary:" -Values @($(if ($passed) { "PASS / PASS / PASS - full quantity, unit price, and amount; ellipsis 0" } else { "unknown" }))
    AddRepoStateSection -Lines $Lines -Title "Alpha.41 Phone Icon-only / 3 Actions:" -Values @($(if ($passed) { "$($manifest.phoneIconOnly) / $($manifest.actionStates.three)" } else { "unknown / unknown" }))
    AddRepoStateSection -Lines $Lines -Title "Alpha.41 Tablet Regression:" -Values @($(if ($passed) { [string]$manifest.tabletResult } else { "unknown" }))
    AddRepoStateSection -Lines $Lines -Title "Alpha.41 Page Footer Count / Texts:" -Values @($(if ($passed) { "$($pdf.footerCount) / $([string]::Join(', ', @($pdf.footerTexts)))" } else { "unknown" }))
    AddRepoStateSection -Lines $Lines -Title "Alpha.41 Header Page Suffix Removal:" -Values @($(if ($passed) { "PASS - suffix count $($pdf.headerSuffixCount)" } else { "unknown" }))
    AddRepoStateSection -Lines $Lines -Title "Alpha.41 Sample PDF:" -Values @($(if ($passed) { "$($pdf.fileSizeBytes) bytes; SHA-256 $($pdf.sha256); $($pdf.pageCount) pages; $([string]::Join('/', @($pdf.pageOrientations))); blank/clipping $($pdf.blankPageCount)/$($pdf.clippingViolationCount)" } else { "unknown" }))
    AddRepoStateSection -Lines $Lines -Title "Alpha.41 Actual PDF Read-only Regression:" -Values @($(if ($passed) { "$($actual.result); R2 GET $($actual.r2GetCount); PUT/DELETE 0/0" } else { "unknown" }))
    AddRepoStateSection -Lines $Lines -Title "Alpha.41 DB / R2 / Token / Generated Document / Production Mutation:" -Values @($(if ($passed) { "$($manifest.dbMutation) / $($manifest.r2Mutation) / $($manifest.tokenMutation) / $($manifest.generatedDocumentMutation) / $($manifest.productionMutation)" } else { "false / false / false / false / false" }))
    AddRepoStateSection -Lines $Lines -Title "Alpha.41 PowerShell Menu Decision:" -Values @("no new menu; static contract remains in automation-infrastructure profile and local PDF/browser QA remains explicit")
}
...
function AddAlpha42RealisticEmbeddedQrRepoStateSections {
    ...
    if ($Version -ne "2.0.0-alpha.42") { return }

    AddRepoStateSection -Lines $Lines -Title "Alpha.42 Completion Status:" -Values @("ALPHA42_RUNTIME_EFFECTS_COMPLETE_STATIC_VALIDATION_PASS")
    AddRepoStateSection -Lines $Lines -Title "Alpha.42 Migration 012 / Ledger:" -Values @("approved dev/test apply exactly once; ledger 12/12; token_purpose column + CHECK + embedded_qr partial unique index")
    AddRepoStateSection -Lines $Lines -Title "Alpha.42 WorkOrder / Revision:" -Values @("issued / finalized; version 2/2")
    AddRepoStateSection -Lines $Lines -Title "Alpha.42 Generated Document:" -Values @("1 retained row; status generated; immutable PDF metadata complete")
    AddRepoStateSection -Lines $Lines -Title "Alpha.42 PDF:" -Values @("252994 bytes; SHA-256 0334727646ebc43ab19a88ccb64cf1b5d3b1e91d3ca5438d3ec61a9a9665af37; 3 pages; landscape/portrait/portrait")
    AddRepoStateSection -Lines $Lines -Title "Alpha.42 Worker-mediated PDF PUT / GET / DELETE:" -Values @("1 / 3 / 0; direct R2/S3 access 0")
    AddRepoStateSection -Lines $Lines -Title "Alpha.42 Finalize / Event Delta:" -Values @("1 / +3")
    AddRepoStateSection -Lines $Lines -Title "Alpha.42 Receipt / Incomplete Receipt:" -Values @("3 / 0")
    AddRepoStateSection -Lines $Lines -Title "Alpha.42 Embedded Token / Access Count:" -Values @("1 / 1; hash-only persistence")
    AddRepoStateSection -Lines $Lines -Title "Alpha.42 Company B Stored Response:" -Values @("PASS - HTTP 404 with top-level WAFL_NOT_FOUND workspace contract")
    AddRepoStateSection -Lines $Lines -Title "Alpha.42 Company H / C Isolation:" -Values @("PASS by static canonical contracts; additional live H/C validation not executed by approved zero-call policy")
    AddRepoStateSection -Lines $Lines -Title "Alpha.42 A30FACT / Production Mutation:" -Values @("unchanged / false")
    AddRepoStateSection -Lines $Lines -Title "Alpha.42 Cleanup / Rollback / Delete:" -Values @("0 / 0 / 0")
    AddRepoStateSection -Lines $Lines -Title "Alpha.42 Remaining Runtime Budget:" -Values @("0 - no additional API, DB, R2, viewer, token, render, or migration call")
    AddRepoStateSection -Lines $Lines -Title "Alpha.42 Viewer Readiness Timing:" -Values @("ephemeral port and exact ready timestamp not preserved / unavailable; no replay")
}
...
function NewLocalRepoBuildResultFile {
    ...
    AddAlpha41MobileSummaryPageNumberRepoStateSections -Lines $lines -Version $Version
    AddAlpha42RealisticEmbeddedQrRepoStateSections -Lines $lines -Version $Version
    ...
    AddRepoStateSection -Lines $lines -Title "Package/Lockfile Changed:" -Values @($(if ($Version -eq "2.0.0-alpha.42") { "mobile version metadata only; dependency graph unchanged" } else { "false" }))
