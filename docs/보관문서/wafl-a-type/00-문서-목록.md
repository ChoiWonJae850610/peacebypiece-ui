...
- `docs/보관문서/wafl-a-type/65-작업지시서-실시간생산-임시저장.md` — 작업지시서 생산구성 숫자 입력값 실시간 draft 반영
...
- [81-작업지시서-생산동기화-게이트.md](./81-작업지시서-생산동기화-게이트.md) — 0.15.58 반려 workflow에서 생산구성 repository sync 차단

- [82-검토요청-생산서비스코드-전달.md](./82-검토요청-생산서비스코드-전달.md) — 0.15.59 검토요청 생산구성 serviceCode 전달 보강

- [83-작업지시서-검토반려-회귀점검.md](./83-작업지시서-검토반려-회귀점검.md) — 0.15.60 검토요청/반려/재검토요청 회귀 테스트 기준

- [84-작업지시서-서비스코드-화면액션점검.md](./84-작업지시서-서비스코드-화면액션점검.md) — 0.15.61 작업지시서 화면 액션별 serviceCode 연결 상태 점검

- [85-작업지시서-서비스코드-1차연결.md](./85-작업지시서-서비스코드-1차연결.md) — 0.15.62 작업지시서 serviceCode 누락 액션 1차 연결

- [86-작업지시서-서비스코드-워크플로우연결.md](./86-작업지시서-서비스코드-워크플로우연결.md) — 0.15.63 작업지시서 workflow/save serviceCode 연결 보강
- [87-공장발주-교체저장.md](./87-공장발주-교체저장.md) — 0.15.64 공장 발주 orders 현재값 replace 저장 정리
- [88-현재생산테이블-SQL설계.md](./88-현재생산테이블-SQL설계.md) — 0.15.65 생산구성 현재값 3개 테이블 컬럼 정리 SQL 설계
- [89-현재생산테이블-스키마구현.md](./89-현재생산테이블-스키마구현.md) — 0.15.66 생산구성 현재값 3개 테이블 schema/repository mapping 1차 반영
