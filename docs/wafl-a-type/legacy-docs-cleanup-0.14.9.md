---
title: WAFL A-TYPE Legacy Docs Cleanup
version: 0.14.9
...
0.14.9부터 프로젝트 문서 기준은 `docs/wafl-a-type/00~20`으로 통일한다.

기존 `docs/` 최상위에 있던 버전별 작업 메모는 다음 이유로 기준 문서에서 제외한다.

```txt
- 버전별 임시 결정이 많아 현재 코드 기준과 충돌할 수 있음
- 같은 주제의 문서가 여러 버전으로 반복됨
- mock/fallback/legacy 전제가 남아 있을 수 있음
- A-TYPE 문서 세트와 중복됨
```

## 삭제 대상

아래 문서들은 0.14.9 patch에서 삭제 대상으로 분류했다. 필요한 내용은 WAFL A-TYPE 00~20 문서로 흡수한다.
