# PeaceByPiece 통계 지표 / 요금제 / 구현 로드맵 (0.9.194)

- 기준 버전: 0.9.1993에서 통계 설계 기준 버전 0.9.194로 재정리
- 문서 버전: 0.9.194
- 목적: 고객관리자/시스템관리자 통계 기능을 요금제별 노출, 데이터 구조, 라이브러리 도입, 성능/DB 설계까지 한 번에 검토할 수 있도록 정리한다.
- 주의: 이 문서는 통계 설계 문서이며, 실제 통계 API/차트 구현은 후속 버전에서 진행한다.
...
### 0.9.199 — 고객관리자 고급 통계 preview/잠금
...
### 0.9.203 — TanStack Query 도입 검토/적용
...
### 0.9.205 — summary table/materialized view 검토
...
0.9.194 문서화
0.9.195 API 설계
0.9.196 인덱스/schema 준비
0.9.197 Recharts 도입
0.9.198 Basic 통계 1차
0.9.199 고급 통계 잠금 preview
0.9.200 Standard/Growth 통계
0.9.201 Premium 통계 준비
0.9.202 시스템관리자 통계
0.9.203 TanStack Query
0.9.204 성능 측정
```
