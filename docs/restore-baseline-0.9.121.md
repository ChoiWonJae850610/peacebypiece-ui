...
1. 먼저 현재 0.9.123 복구 라인의 build와 화면을 기준으로 삼는다.
...
첨부 다운로드/미리보기 안정화 필요 여부를 실제 동작 기준으로 확인한다. 수정이 필요하면 `attachmentFileRoute`, Worker file URL, 다운로드 query 처리 범위로 제한한다.
