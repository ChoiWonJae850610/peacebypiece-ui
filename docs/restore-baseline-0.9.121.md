...
1. 먼저 현재 0.9.128 복구 라인의 build와 화면을 기준으로 삼는다.
...
작업지시서 핵심 flow 회귀 테스트 문서를 추가한다. 작성중, 검토요청, 검토완료, 발주요청, 생산/검수, 완료 상태 변경 후 주요 데이터 유지 여부를 확인한다.
