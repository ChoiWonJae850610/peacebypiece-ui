...
1. 먼저 현재 0.9.125 복구 라인의 build와 화면을 기준으로 삼는다.
...
첨부 업로드 로그와 Worker 경로를 점검한다. R2 직접 SDK 업로드/삭제 방식으로 되돌리지 않고, upload/direct 500 및 SSL EPROTO 로그가 실제 사용자 기능에 영향을 주는지 확인한다.

### 0.9.127

첨부 삭제/복구/대표 이미지 흐름을 점검한다. 작업지시서 일반 삭제와 관리자 파일관리 영구삭제/R2 물리 삭제를 분리해서 확인한다.
