...
- 0.19.94.8: docs 루트 보관 후보 185개를 `docs/보관문서/` 하위로 이동하고 문서 루트 밀도를 낮춤.

- 0.19.94.7: docs 루트 수동 검토 후보 9개를 내용 기준으로 재분류하고 모두 보관 후보로 확정.
...
현재 개발·테스트·리팩토링에서 우선 확인해야 하는 문서는 `docs/현재기준/`에 둔다. 과거 WAFL UI 전환 기록은 `docs/보관문서/wafl-a-type/`으로 이동했다.
...
- 보관 이동 대상: 185개
- 이동 후 루트에는 최근 운영·정책·DB/API smoke·Playwright·회사 파일 업로드 설계 등 현행 기준 문서만 남기는 방향으로 정리한다.
- 이번 이동은 문서 보관 이동이며 코드/DB/API/R2 동작에는 영향이 없다.

상세 목록: `docs/docs-archive-move-0.19.94.8.md`
