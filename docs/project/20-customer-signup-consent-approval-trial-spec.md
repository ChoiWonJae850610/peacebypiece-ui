...
가입자는 가입 요청 중 유료 요금제를 선택하고 카드 등록을 완료한다. 카드 등록과 결제수단 유효성 확인이 완료되지 않은 요청은 승인할 수 없다. Trial은 시스템 관리자가 가입 요청을 승인한 순간 시작한다.
...
- 하나의 고객사 관리자 이메일은 하나의 회사에만 귀속된다. 복수 회사 관리자 상태는 허용하지 않는다.
...
- 초기 출시에는 마케팅 이메일 프로그램을 운영하지 않는다.
- 인증, 승인, 결제, 보안, 정책, 장애, 운영 필수 안내만 발송한다.
- 향후 마케팅 기능을 도입하면 별도 선택 동의와 철회 기능을 추가한다.
...
| `changes_requested` | 보완 요청 | 3일 안에 수정 및 재제출; 기한 경과 시 자동 거절 |
...
사업자등록증은 승인 전용 viewer에서만 제한 열람하며 다운로드를 금지한다. 접근 목적·사유·접근자·시각 로그를 남긴다. 일반 고객 업무 콘텐츠는 열람하지 않는다.
...
- 저장공간: 100MB
- 멤버: 3명
- 데이터 내보내기: 제한된 회사 전체 Export
- 가입 시 유료 요금제를 선택한다.
- 가입 시 카드 등록과 결제수단 유효성 확인을 완료한다.
- 시스템 관리자 승인 순간 Trial이 시작된다.
- 가입 직후, 결제 3일 전, 결제 1일 전에 자동결제 안내 이메일을 보낸다.
- Trial 중 해지해도 종료일까지 사용한다.
- 해지하지 않으면 Trial 종료 시 선택한 요금제로 자동 결제한다.
- 카드 원문은 저장하지 않고 PG token/billing key만 저장한다.
- PG 연동과 세금계산서 구현은 사업자등록 후 별도 Sprint로 진행한다.
