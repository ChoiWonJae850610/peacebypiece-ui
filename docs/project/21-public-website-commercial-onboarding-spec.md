...
- public analytics는 authenticated app의 고객 업무 이벤트를 기본 수집하지 않는다.
...
- 문의 채널
- Trial CTA 문구
- 공개 전 사용할 제품 캡처 최종 목록
- public website와 app을 같은 repository/deployment로 운영할지 분리할지
- cookie banner와 third-party analytics 도입 여부

이 결정 전에도 sitemap, route boundary, copy inventory, screenshot requirements, Codex implementation order는 준비할 수 있다.
