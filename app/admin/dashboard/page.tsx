...
export default function AdminDashboardPage() {
  const maxFlowValue = getAdminStatMaxFlowValue();
  const totalPartnerCount = getAdminStatTotalPartnerCount();
        ...
        <AdminCard className="flex min-h-0 flex-col overflow-hidden">
          <div className="flex items-start justify-between gap-3 border-b border-stone-100 pb-4">
            <div>
              ...
              <p className="mt-1 text-xs text-stone-500">mock 데이터 기준 상태별 작업량</p>
            </div>
          ...
          <div className="mt-5 flex min-h-0 flex-1 items-end gap-4 rounded-[24px] border border-stone-100 bg-stone-50/70 px-5 pb-5 pt-8">
            {ADMIN_STAT_WORKORDER_FLOW.map((item) => {
        ...
        <div className="grid min-h-0 gap-5 overflow-hidden">
          <AdminCard className="min-h-0">
            ...
            <div className="mt-5 grid gap-3">
              {ADMIN_STAT_PARTNER_DISTRIBUTION.map((item) => {
          ...
          <AdminCard className="min-h-0">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="rounded-full bg-stone-100 px-3 py-1 text-xs font-semibold text-stone-500">mock</span>
            </div>
            <div className="mt-5 grid gap-3">
              {ADMIN_STAT_FILE_USAGE_POINTS.map((item) => {
                const width = Math.min(100, Math.round((item.value / item.limit) * 100));
                return (
