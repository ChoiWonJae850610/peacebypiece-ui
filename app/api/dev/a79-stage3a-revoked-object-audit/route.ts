...
export async function POST(request: Request) {
  ...
  let config;
  try { config = readExternalQaServerConfig(process.env); } catch { return NextResponse.json({ error: "EXTERNAL_QA_DISABLED" }, { status: 403 }); }
  if (!config.enabled || config.production || !isMakerQaCapabilityEnabled(process.env, MAKER_QA_CAPABILITY.DOCUMENT_R0)) {
  ...
  if (!/^[0-9a-f]{8}-[0-9a-f-]{27}$/iu.test(documentId)) return NextResponse.json({ error: "REQUEST_INVALID" }, { status: 400 });
  const scope = createCommandTenantScope({ scope: guard.scope, companyMemberId: guard.session.companyMemberId,
    permissionCode: "workorder.read",
  ...
  const row = await withWaflV2TenantReadOnlyTransaction(async (client) => {
    ...
    return (await client.query<DbQueryResultRow>(`
      SELECT d.id::text,d.status,d.revoked_at,d.storage_object_key,d.file_size_bytes,d.content_sha256,w.product_name
      ...
      WHERE d.company_id=$1 AND d.id=$2::uuid AND d.deleted_at IS NULL
      LIMIT 1
    `, [guard.scope.companyId, documentId])).rows[0] ?? null;
  });
  if (!row || !FIXTURES.has(String(row.product_name)) || row.status !== "revoked" || row.revoked_at === null) {
    return NextResponse.json({ error: "EXACT_REVOKED_FIXTURE_REQUIRED" }, { status: 409 });
  }
  const health = await inspectGeneratedDocumentArtifact({ documentId, objectKey: String(row.storage_object_key),
    fileSizeBytes: Number(row.file_size_bytes), contentSha256: String(row.content_sha256) });
  return NextResponse.json({ ok: true, data: { documentRef: safeRef(documentId), objectKeyRef: safeRef(String(row.storage_object_key)),
    lifecycle: "revoked", objectHealth: health, objectPresent: health === "healthy", r2Delete: 0, unrelatedMutation: 0 } },
