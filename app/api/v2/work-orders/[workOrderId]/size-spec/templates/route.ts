...
import { listCompatibleMeasurementTemplates } from "@/lib/domain/work-orders/measurement/templateRepository";
...
export async function GET(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  return createWaflApiSuccess({ workOrderId: (await context.params).workOrderId, items }, { headers: { "Cache-Control": "no-store", "X-WAFL-Correlation-Id": correlationId } });
}
