...
export async function POST(request: NextRequest) {
  if (!isR2Configured()) {
    return NextResponse.json({ uploadTargets: [], error: "R2_NOT_CONFIGURED" }, { status: 503 });
  }
  ...
  try {
    ...
    if (!workOrderId) {
      return NextResponse.json({ uploadTargets: [], error: "WORK_ORDER_ID_REQUIRED" }, { status: 400 });
    }

    if (files.length === 0) {
      return NextResponse.json({ uploadTargets: [], error: "FILES_REQUIRED" }, { status: 400 });
    }
    ...
    const uploadTargets = files.map((file) => {
      ...
      const presigned = createR2PresignedPutUrl({
        key: storageKey,
