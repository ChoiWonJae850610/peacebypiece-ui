...
- 현재 작업 상태: 0.19.94.9 기준으로 docs 보관 이동 후 루트 잔여 문서와 README 링크 정리 기준을 점검하고, 기능 코드/DB/API/R2 흐름은 변경하지 않는다.
...
## 0.19.94.9 docs 이동 후 잔여 문서 점검

- `docs/` 루트에는 최근 운영·정책·DB/API smoke·Playwright·회사 파일 업로드 설계 관련 문서 44개가 남아 있다.
- 0.19.94.8에서 이동한 과거 문서 링크는 `docs/README.md`의 긴 직접 링크 목록에서 제거하고, 보관문서 분류 기준으로 안내한다.
- 기능 코드, DB schema, API route, R2 업로드 흐름은 변경하지 않는다.

관련 문서: `docs/docs-root-residual-audit-0.19.94.9.md`
